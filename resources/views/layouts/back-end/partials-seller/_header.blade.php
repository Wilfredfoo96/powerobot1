<div id="headerMain" class="d-none">
    <header id="header"
        ...
        <div class="navbar-nav-wrap">
            <div class="navbar-brand-wrapper">
                ...
                <a class="navbar-brand" href="{{route('seller.dashboard.index')}}" aria-label="">
                    ...
                    <img class="navbar-brand-logo"
                        onerror="this.src='{{asset('assets/back-end/img/160x160/img1.jpg')}}'" src="{{asset("
                        storage/shop/$shop->image")}}" alt="Logo" height="40">
                    <img class="navbar-brand-logo-mini"
                        onerror="this.src='{{asset('assets/back-end/img/160x160/img1.jpg')}}'" src="{{asset("
                        storage/shop/$shop->image")}}"
                    alt="Logo" height="40">
            ...
            <!-- Secondary Content -->
            <div class="navbar-nav-wrap-content-right" style="{{Session::get('direction') === " rtl"
                ? 'margin-left:unset; margin-right: auto' : 'margin-right:unset; margin-left: auto' }}">
                <!-- Navbar -->
                    ...
                    <li class="nav-item d-none d-md-inline-block">
                        <div class="hs-unfold">
                            <div>
                                ...
                                <div class="topbar-text dropdown disable-autohide {{Session::get('direction') === " rtl"
                                    ? 'ml-3' : 'm-1' }} text-capitalize">
                    ...
                    <li class="nav-item d-none d-md-inline-block">
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle-left-arrow dropdown-toggle-empty"
                                ...
                                @php($notification=App\Model\Notification::whereBetween('created_at',
                                [auth('seller')->user()->created_at,
                                \Illuminate\Support\Carbon::now()])->where('sent_to',
                                'seller')->whereDoesntHave('notification_seen_by')->count())
                                @if($notification!=0)
                                <span class="btn-status btn-sm-status btn-status-danger notification_data_new_count">{{
                                    $notification }}</span>
                                @endif
                            </a>
                            <div id="notificationDropdown"
                                ...
                                @php($notification_data=App\Model\Notification::whereBetween('created_at',
                                [auth('seller')->user()->created_at,
                                \Illuminate\Support\Carbon::now()])->where('sent_to',
                                'seller')->with('notification_seen_by')->latest()->get())

                                @foreach ($notification_data as $item)
                                <button class="dropdown-item position-relative"
                                    onclick="notification_data_view({{ $item->id }})">
                    ...
                    <li class="nav-item d-none d-md-inline-block">
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle dropdown-toggle-left-arrow"
                                ...
                                <i class="tio-email"></i>
                                @php($message=\App\Model\Chatting::where(['seen_by_seller'=>0,
                                'seller_id'=>auth('seller')->id()])->count())
                                @if($message!=0)
                            ...
                            <div id="messageDropdown"
                                ...
                                <a class="dropdown-item position-relative"
                                    ...
                                    @php($message_customer=\App\Model\Chatting::where(['seen_by_seller'=>0,
                                    'seller_id'=>auth('seller')->id()])->whereNotNull(['user_id'])->count())
                                ...
                                <a class="dropdown-item position-relative"
                                    ...
                                    @php($message_d_man =\App\Model\Chatting::where(['seen_by_seller'=>0,
                                    'seller_id'=>auth('seller')->id()])->whereNotNull(['delivery_man_id'])->count())
                                    @if($message_d_man > 0)
                                    <span class="btn-status btn-sm-status-custom btn-status-danger">{{ $message_d_man
                                        }}</span>
                                    @endif
                    ...
                    <li class="nav-item d-none d-md-inline-block">
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                ...
                                @php($order=\App\Model\Order::where(['seller_is'=>'seller','seller_id'=>auth('seller')->id(),
                                'order_status'=>'pending'])->count())
                                @if($order!=0)
        ...
        <div id="website_info" style="display:none;" class="bg-secondary w-100">
            <div class="p-3">
                <div class="bg-white p-1 rounded">
                    @php( $local = session()->has('local')?session('local'):'en')
                    @php($lang = \App\Model\BusinessSetting::where('type', 'language')->first())
                    <div class="topbar-text dropdown disable-autohide {{Session::get('direction') === " rtl" ? 'ml-3'
                        : 'm-1' }} text-capitalize">
