...
<table>
    <thead>
        ...
        <tr>
            ...
            <th></th>
            <th>
                {{translate('category')}} - {{$data['category'] != 'all' ? $data['category']['defaultName'] :
                $data['category'] }}
                <br>
                {{translate('sub_Category')}} - {{$data['sub_category'] != 'all' ? $data['sub_category']['defaultName']
                : $data['sub_category'] }}
                <br>
                {{translate('sub_Sub_Category')}} - {{$data['sub_sub_category'] != 'all' ?
                $data['sub_sub_category']['defaultName'] : $data['sub_sub_category'] }}
                <br>
                ...
                <br>
                {{translate('status')}} - {{translate($data['status']==0 ? 'pending' : ($data['status'] == 1 ?
                'approved' : 'denied') )}}
                @endif
                <br>
        ...
        <tr>
            ...
            <td>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($item['unit_price']))}}
            </td>
