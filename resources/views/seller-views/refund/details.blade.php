            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap flex-column flex-md-row gap-10 justify-content-between">
                        <span class="title-color">{{translate('subtotal')}} :
                            {{\App\CPU\Helpers::currency_converter($subtotal)}}</span>
                        <span class="title-color">{{translate('coupon_discount')}} :
                            {{\App\CPU\Helpers::currency_converter($coupon_discount)}}</span>
                        <span class="title-color">{{translate('total_refund_amount')}} :
                ...
                <div class="card-body">
                    <div class="row gy-2">
                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            ...
                            <span>{{translate('deliveryman_name')}} :
                                {{$order->delivery_man!=null?$order->delivery_man->f_name . ' '
                                .$order->delivery_man->l_name:translate('not_assigned')}}</span>
                            <span>{{translate('deliveryman_email')}} :
                                {{$order->delivery_man!=null?$order->delivery_man->email
                                :translate('not_found')}}</span>
                            <span>{{translate('deliveryman_phone')}} :
                                {{$order->delivery_man!=null?$order->delivery_man->phone
                                :translate('not_found')}}</span>
                        </div>

                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            <span>{{translate('payment_method')}} : {{str_replace('_','
                                ',$order->payment_method)}}</span>
                            <span>{{translate('order_details')}} : <a class="btn btn--primary btn-sm"
                                    href="{{route('seller.orders.details',['id'=>$order->id])}}">{{translate('click_here')}}</a></span>
                        </div>
                    </div>
                </div>
            </div>
