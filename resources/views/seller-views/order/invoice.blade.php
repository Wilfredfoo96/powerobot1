        ...
        <table class="bs-0 mb-30 px-10">
            <tr>
                ...
                <th class="content-position-y text-right">
                    <h4 class="fz-14">{{ucwords('date')}} : {{date('d-m-Y h:i:s a',strtotime($order['created_at']))}}
                    </h4>
                </th>
            </tr>
        </table>
                                ...
                                <td>
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        <p style=" margin-top: 6px; margin-bottom:0px;">
                                            {{$shipping_address->contact_person_name}}</p>
                                        ...
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$shipping_address->address}}
                                        </p>
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{ $shipping_address->city }} {{
                                            $shipping_address->zip }} </p>
                                    </div>
                                    @else
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        ...
                                        @else
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{ $order->customer !=null?
                                            $order->customer['f_name'].' '.$order->customer['l_name']:'Name not found'
                                            }}</p>
                                        @endif

                                        @if (isset($order->customer) && $order->customer['id']!=0)
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null?
                                            $order->customer['email']: 'Email not found' }}</p>
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->customer !=null?
                                            $order->customer['phone']: 'Phone not found' }}</p>
                                        @endif
                                    </div>
                                    @endif
                                </td>
                        ...
                        <table>
                            <tr>
                                <td class="text-right">
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        <p style=" margin-top: 6px; margin-bottom:0px;">
                                            {{$billingAddress->contact_person_name}}</p>
                                        ...
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$billingAddress->city}}
                                            {{$billingAddress->zip}}</p>
                                    </div>
                                    @elseif($order->billingAddress)
                                    <span class="h2">Billing Address </span>
                                    <div class="h4 montserrat-normal-600">
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->billingAddress ?
                                            $order->billingAddress['contact_person_name'] : ""}}</p>
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->billingAddress ?
                                            $order->billingAddress['phone'] : ""}}</p>
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->billingAddress ?
                                            $order->billingAddress['address'] : ""}}</p>
                                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->billingAddress ?
                                            $order->billingAddress['city'] : ""}} {{$order->billingAddress ?
            ...
            <table class="customers bs-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details['price']))}}
                        </td>
                        <td>{{$details->qty}}</td>
                        <td class="text-right">
            ...
            <tr>
                <th class="text-left" style="width: 60%">
                    ...
                    <p class="fz-12 font-normal">
                        ...
                        <br>
                        <span>
                            {{ucwords('deliveryman name')}} : {{$order->delivery_man['f_name'].'
                            '.$order->delivery_man['l_name']}}
                        </span>
                        <br>
                        <span>
                    ...
                    <table class="calc-table">
                        <tbody>
                            <tr>
                                ...
                                <td class="p-1">
                                    {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sub_total))}}
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="p-1">
                                    ...
                                    - ($order['is_shipping_free'] ? $order['extra_discount'] : 0)))}}</td>
                            </tr>
                            @endif
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="p-1">
                                    -
                                    {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($ext_discount))}}
                                </td>
                            </tr>
                            @endif
                            <tr>
                            ...
                            <tr>
                                <td class="border-dashed-top font-weight-bold text-left"><b>{{ucwords('total')}}</b>
                                </td>
            ...
            <table>
                <tr>
                    <th class="fz-12 font-normal pb-3">
                        If you require any assistance or have feedback or suggestions about our site, you <br /> can
                        email us at <a href="mailto:{{ $company_email }}">{{ $company_email }}</a>
                    </th>
                </tr>
                <tr>
                    <th class="content-position-y bg-light py-4">
                        ...
                        <div>
                            {{ucwords('all copyright reserved ©'.date('Y').'_').$company_name}}
                        </div>
                    </th>
                </tr>
            </table>
