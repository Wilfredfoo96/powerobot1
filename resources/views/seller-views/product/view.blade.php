            ...
            <div class="d-flex flex-wrap flex-lg-nowrap gap-3 justify-content-between">
                <div class="media flex-wrap flex-sm-nowrap gap-3">
                    <a class="aspect-1 float-left overflow-hidden"
                        @if(file_exists(base_path("storage/product/thumbnail/".$product['thumbnail'])))
                        href="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$product['thumbnail']}}"
                        @else href="{{asset(" assets/front-end/img/image-place-holder.png")}}" @endif
                    ...
                    <div class="d-block">
                        <div class="d-flex flex-wrap flex-sm-nowrap align-items-start gap-2 mb-2 min-h-50">
                            ...
                            <a class="aspect-1 float-left overflow-hidden"
                                @if(file_exists(base_path("storage/product/".$photo->image_name)))
                                href="{{asset("storage/product/$photo->image_name")}}"
                                @else
                                ...
                                <img width="50"
                                    onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'"
                                    src="{{asset(" storage/product/$photo->image_name")}}" alt="Product
                                image">
                            </a>
                            @endforeach
                            @else
                            @foreach (json_decode($product->images) as $key => $photo)
                            <a class="aspect-1 float-left overflow-hidden" href="{{asset(" storage/product/$photo")}}"
                                data-lightbox="mygallery">
                                <img width="50"
                                    onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'"
                        ...
                        <div class="d-block">
                            ...
                            <div class="d-flex gap-3 flex-wrap mb-3 lh-1">
                                <a href="#" class="text-dark">{{count($product->order_details)}}
                                    {{translate('orders')}}</a>
                                <span class="border-left"></span>
                                <div
                                    class="review-hover position-relative cursor-pointer d-flex gap-2 align-items-center">
                                    <i class="tio-star"></i>
                                    <span>{{count($product->rating)>0?number_format($product->rating[0]->average, 2,
                                        '.', ' '):0}}</span>
                                            ...
                                            <ul class="list-unstyled list-unstyled-py-2 mb-0">
                                                ...
                                                <li class="d-flex align-items-center font-size-sm">
                                                    ...
                                                    <span class="{{Session::get('direction') === " rtl" ? 'ml-3'
                                                        : 'mr-3' }}">{{translate('5')}} {{ translate('star') }}</span>
                                ...
                                <span class="border-left"></span>
                                <a href="javascript:" class="text-dark">{{$product->reviews_count}}
                                    {{translate('ratings')}}</a>
                                <span class="border-left"></span>
                                <a href="javascript:"
                                    class="text-dark">{{$product->reviews->whereNotNull('comment')->count()}}
                                    {{translate('reviews')}}</a>
                            </div>
                            ...
                            file_exists(base_path('storage/product/digital-product/'.$product->digital_file_ready)))
                ...
                <div class="border p-3 w-170">
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <h3 class="text-primary fs-18">
                            {{\App\CPU\Helpers::currency_converter(($product->order_delivered_sum_price *
                            $product->order_delivered_sum_qty) - $product->order_delivered_sum_discount)}}</h3>
                    </div>
                </div>
                ...
                <div class="row gy-3 flex-grow-1">
                    <div class="col-sm-6 col-xl-4">
                        ...
                        <div class="pair-list">
                            ...
                            <div>
                                <span class="key">{{translate('product_type')}}</span>
                                <span>:</span>
                            ...
                            @endif
                            <div>
                                <span class="key">{{translate('SKU')}}</span>
                                <span>:</span>
                    ...
                    <div class="col-sm-6 col-xl-4">
                        ...
                        <div class="pair-list">
                            <div>
                                <span class="key">{{translate('purchase_price')}}</span>
                                <span>:</span>
                                <span
                                    class="value">{{\App\CPU\Helpers::currency_converter($product->purchase_price)}}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <span>:</span>
                                <span
                                    class="value">{{\App\CPU\Helpers::currency_converter($product->unit_price)}}</span>
                            </div>
                            ...
                            <div>
                                ...
                                @else
                                <span class="value">{{$product->tax}}% ({{$product->tax_model}})</span>
                                <span class="value">{{\App\CPU\Helpers::currency_converter($product->tax)}}%
                                    ({{$product->tax_model}})</span>
                                @endif
                            </div>
                            @if($product->product_type == 'physical')
                            <div>
                                <span class="key">{{translate('shipping_cost')}}</span>
                                <span>:</span>
                                <span class="value">
                            ...
                            <div>
                                <span class="key">{{translate('discount')}}</span>
                                <span>:</span>
                    ...
                    @if ($product->product_type == 'physical' && count(json_decode($product->choice_options)) >0 ||
                    count(json_decode($product->colors)) >0 )
                    <div class="col-sm-6 col-xl-4">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="text-wrap max-w-400 min-w-200">
                                ...
                                <a class="aspect-1 float-left overflow-hidden"
                                    href="{{asset('storage/review')}}/{{$img}}" data-lightbox="mygallery">
                                    <img class="p-2" width="60" height="60" src="{{asset('storage/review')}}/{{$img}}"
                                        alt=""
                                        onerror="this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'">
                                </a>
                                @endforeach
                        ...
                        <td>
                            <form
                                action="{{ route('seller.reviews.status', [$review['id'], $review->status ? 0 : 1]) }}"
                                method="get" id="reviews_status{{$review['id']}}_form" class="reviews_status_form">
                                <label class="switcher mx-auto">
                                    <input type="checkbox" class="switcher_input" id="reviews_status{{$review['id']}}"
                                        {{ $review->status ? 'checked' : '' }}
                                    onclick="toogleStatusModal(event,'reviews_status{{$review['id']}}','customer-reviews-on.png','customer-reviews-off.png','{{translate('Want_to_Turn_ON_Customer_Reviews')}}','{{translate('Want_to_Turn_OFF_Customer_Reviews')}}',`
                                    <p>{{translate('if_enabled_anyone_can_see_this_review_on_the_user_website_and_customer_app')}}
                                    </p>`,`<p>
                                        {{translate('if_disabled_this_review_will_be_hidden_from_the_user_website_and_customer_app')}}
                                    </p>`)">
