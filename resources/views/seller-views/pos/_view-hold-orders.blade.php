...
<div class="table-responsive datatable-custom custom-scrollbar-pos">
    <table class="table table-hover table-thead-bordered table-nowrap table-align-middle card-table w-100 min-h-300"
        ...
        <tbody>
            ...
            @if(isset($customer) && $customer !== '' ? strpos(strtolower($customer_name), strtolower($customer)) !==
            false : $customer_name)
            <tr>
                        ...
                        <div class="bg-white p-0 box-shadow table-items-popup">
                            ...
                            <div class="p-3 border-bottom rounded d-flex justify-content-between gap-2">
                                <div class="media gap-2">
                                    <img width="40" src="{{asset('storage/product/thumbnail')}}/{{$cartItem['image']}}"
                                        onerror="this.src='{{asset('assets/back-end/img/160x160/img2.jpg')}}'">
                                    <div class="media-body">
                                        <h6 class="text-truncate"> {{ Str::limit($cartItem['name'], 12 )}}</h6>
                                        <div class="text-muted">{{ translate('qty') }}: {{ $cartItem['quantity'] }}
                                        </div>
                                    </div>
                                </div>
                                <h5>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($product_subtotal))}}
                                </h5>
                            </div>
                            @endif
                            @endforeach
                ...
                <td>
                    ...
                    <del>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($subtotal,
                        2)))}}</del>
                    @endif
                    {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($total+$total_tax-$coupon_discount,
                    2)))}}
                </td>
                <td>
                    <div class="d-flex justify-content-center gap-2">
                        <button type="button" class="btn btn-soft-warning"
                            onclick="cart_change('{{ $single_cart }}')">{{translate('resume')}}</button>
                        <button type="button" class="btn btn-soft-danger"
                            onclick="cancel_customer_order('{{ $single_cart }}')">{{translate('cancel_order')}}</button>
                    </div>
                </td>
            </tr>
            @endif
