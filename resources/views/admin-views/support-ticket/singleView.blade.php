        ...
        <div class="card-header flex-wrap gap-3">
            ...
            <div class="media d-flex gap-3">
                ...
                <div class="media-body">
                    <h6 class="font-size-md mb-1">{{isset($userDetails)?$userDetails['f_name'].'
                        '.$userDetails['l_name']:'not found'}}</h6>
                    <div class="fz-12">{{isset($userDetails)?$userDetails['phone']:''}}</div>
                </div>
            </div>
            <div class="d-flex align-items-center flex-wrap gap-3">
                <div class="type font-weight-bold bg-soft--primary c1 px-2 rounded">{{translate(str_replace('_','
                    ',$ticket['type']))}}</div>
                <div class="priority d-flex flex-wrap align-items-center gap-3">
                    <span class="title-color">{{translate('priority')}}:</span>
                    <span class="font-weight-bold badge-soft-info rounded px-2">{{translate(str_replace('_','
                            ...
                            <div class="sent_msg">
                                <div class="received_withd_msg">
                                        ...
                                        <div class="col-6 col-md-2 position-relative img_row{{$index}}">
                                            <a data-lightbox="mygallery" href="{{asset("
                                                ...
                                                <img onerror="this.src=' {{asset('assets/back-end/img/image-place-holder.png')}}'"
                                                    src="{{asset('storage/support-ticket/'.$photo)}}" alt=""
                                                    class="img-fit">
                                            </a>
                                        </div>
                                        @endforeach
                                    ...
                                    <span class="time_date fz-12 pt-2 d-flex justify-content-end">
                                        {{$message->created_at->format('h:i A')}} | {{$message->created_at->format('M
                                        d')}} </span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row g-2 flex-wrap pt-1 justify-content-start">
                                    ...
                                    <div class="col-6 col-md-2 position-relative img_row{{$index}}">
                                        <a data-lightbox="mygallery" href="{{asset(" storage/chatting/".$photo)}}"
                                            class="aspect-1 overflow-hidden d-block border rounded">
                                            <img onerror="this.src=' {{asset('assets/back-end/img/image-place-holder.png')}}'"
                                                src="{{asset('storage/chatting/'.$photo)}}" alt="" class="img-fit">
                                        </a>
                                    </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="received_msg">
                                <div class="received_withd_msg">
                                    ...
                                    <span class="time_date fz-12 pt-2 d-flex justify-content-start">
                                        {{$ticket->created_at->format('h:i A')}} | {{$ticket->created_at->format('M
                                        d')}} </span>
                                </div>
                            </div>
                        </div>
                            ...
                            <form class="needs-validation" href="{{route('admin.support-ticket.replay',$reply['id'])}}"
                                ...
                                <div class="position-relative d-flex align-items-center">
                                    ...
                                    <label
                                        ...
                                        <input type="file" id="msgfilesValue" class="h-100 position-absolute w-100 "
                                            hidden multiple accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                    </label>
                                    @endif

                                    <textarea class="form-control {{ theme_root_path() == " default" ? 'pl-8' :'' }}"
                                        id="msgInputValue" name="replay" type="text"
