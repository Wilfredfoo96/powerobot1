                        ...
                        <div class="">
                            <div class="d-flex flex-wrap flex-sm-nowrap gap-3 justify-content-end">
                                ...
                                <select class="form-control border-color-c1 w-160"
                                    ...
                                    <option value="high" {{$priority=='high' ?'selected':''}}>{{translate('high')}}
                                    </option>
                                ...
                                <select class="form-control border-color-c1 w-160"
                                    ...
                                    <option value="close" {{$status=='close' ?'selected':''}}>{{translate('close')}}
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div
                            ...
                            <div class="media gap-3">
                                ...
                                <div class="media-body">
                                    <h6 class="mb-0 {{Session::get('direction') === " rtl" ? 'text-right' : 'text-left'
                                        }}">{{$ticket->customer->f_name??""}} {{$ticket->customer->l_name??""}}</h6>
                                    <div class="mb-2 fz-12 {{Session::get('direction') === " rtl" ? 'text-right'
                                        : 'text-left' }}">{{$ticket->customer->email??""}}</div>
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        <span
                                            class="badge-soft-danger fz-12 font-weight-bold px-2 radius-50">{{translate(str_replace('_','
                                            ',$ticket->priority))}}</span>
                                        <span
                                            class="badge-soft-info fz-12 font-weight-bold px-2 radius-50">{{translate(str_replace('_','
                                            ',$ticket->status))}}</span>
                                        <h6 class="mb-0">{{translate(str_replace('_',' ',$ticket->type))}}</h6>
                                        <div class="text-nowrap {{Session::get('direction') === " rtl" ? 'pr-9' : 'pl-9'
                                            }}">
                                            @if ($ticket->created_at->diffInDays(\Carbon\Carbon::now()) < 7) {{ date('D
                                                h:i:A',strtotime($ticket->created_at)) }}
                                                @else
                            ...
                            <form action="{{route('admin.support-ticket.status')}}" method="post"
                                id="support_ticket{{$ticket['id']}}_form" class="support_ticket_form">
                                @csrf
                                <input type="hidden" name="id" value="{{$ticket['id']}}">
                                <label class="switcher mx-auto">
                                    <input type="checkbox" class="switcher_input" id="support_ticket{{$ticket['id']}}"
                                        name="status" value="{{ $ticket['status'] == 'open' ? 'close':'open' }}" {{
