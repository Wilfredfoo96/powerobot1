            ...
            <div class="card">
                <div class="card-body py-5">
                    ...
                    <div class="d-flex justify-content-between gap-3 flex-wrap mb-5">
                        ...
                        <div>
                            <select name="for_customer" id="for_customer" class="form-control min-w-200 text-capitalize"
                                onchange="select_user_type(this.value)">
                    ...
                    <div class="customer_view">
                        <form action="{{route('admin.notification.update-push-notification',['type'=>'customer'])}}"
                                    ...
                                    <div class="form-group">
                                        <div
                                            ...
                                            <label for="customer{{$value['key']}}" class="switcher_content">
                                                ...
                                                'order_out_for_delivery_message' : ($value['key'] == 'order_canceled' ?
                                                'order_canceled_message' : $value['key'])) }}
                                            </label>
                                            <label class="switcher" for="customer{{$value['key']}}">
                                                <input type="checkbox" class="switcher_input"
                                                    name="status{{$value['id']}}" id="customer{{$value['key']}}"
                                        ...
                                        <textarea name="message{{$value['id']}}[]"
                                            class="form-control {{ $lang != $default_lang ? 'd-none' : '' }} lang_form {{ $lang }}-form">{{$translate[$lang][$value['key']]??$value['message']}}</textarea>
                                        @endforeach
                                    </div>
                                </div>
                                @endforeach
                            ...
                            <div class="d-flex gap-3 justify-content-end">
                                <button type="reset" onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}"
                    ...
                    <div class="seller_view d-none">
                        <form action="{{route('admin.notification.update-push-notification',['type'=>'seller'])}}"
                            class="text-start" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        <div
                                            ...
                                            <label class="switcher" for="seller{{$value['key']}}">
                                                <input type="checkbox" class="switcher_input"
                                                    name="status{{$value['id']}}" id="seller{{$value['key']}}" value="1"
                    ...
                    <div class="delivery_man_view d-none">
                        <form action="{{route('admin.notification.update-push-notification',['type'=>'delivery_man'])}}"
                            class="text-start" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        <div
                                            ...
                                            <label class="switcher" for="delivery_man{{$value['key']}}">
                                                <input type="checkbox" class="switcher_input"
                                                    name="status{{$value['id']}}" id="delivery_man{{$value['key']}}"
                                                    value="1" {{$value['status']==1?'checked':''}}
...
{{-- <div class="modal fade" id="orderPendingConfirmModal" tabindex="-1" aria-labelledby="orderPendingConfirmModal"
    aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body px-4 px-sm-5 pt-0 text-center">
                <div class="d-flex flex-column align-items-center gap-2">
                    <img width="80" class="mb-3" src="{{asset('/assets/back-end/img/notice.png')}}" loading="lazy"
                        alt="">
                    <h4 class="lh-md">Warning!</h4>
                    <p>Disabling mail configuration services will prevent the system from sending emails. Please only
                        turn off this service if you intend to temporarily suspend email sending. Note that this may
                        affect system functionality that relies on email communication."</p>
                </div>
                <div class="d-flex justify-content-center gap-3 mt-3">
                    <button type="button" class="btn btn--primary min-w-120 px-5"
                        data-dismiss="modal">{{translate('ok')}}</button>
                    <button type="button" class="btn btn-danger-light min-w-120 px-5"
            ...
            <div class="modal-body px-4 px-sm-5 pt-0">
                <div class="d-flex flex-column gap-2">
                    <div class="text-center mb-1">
                        <img width="80" class="mb-4" src="{{asset('/assets/back-end/img/notice.png')}}" loading="lazy"
                            alt="">
                        <h4 class="lh-md text-capitalize">{{translate('important_notice')}}!</h4>
                    </div>
                    <p class="mb-5">
                        {{translate('to_include_specific_details_in_your_push_notification_message,you_can_use_the_following_placeholders')}}:
                    </p>

                    <ul class="d-flex flex-column px-4 gap-2 mb-4">
                        <li><strong>
                                <?= '{deliveryManName}'?>:
                            </strong> {{translate('the_name_of_the_delivery_person')}}.</li>
                        <li><strong>
                                <?= '{orderId}'?>:
                            </strong> {{translate('the_unique_ID_of_the_order')}}.</li>
                        <li><strong>
                                <?= '{time}'?>:
                            </strong> {{translate('the_expected_delivery_time')}}.</li>
                        <li><strong>
                                <?= '{userName}'?>:
                            </strong> {{translate('the_name_of_the_user_who_placed_the_order')}}.</li>
                    </ul>
