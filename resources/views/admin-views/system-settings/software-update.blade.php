            ...
            <div class="card">
                <div class="border-bottom px-4 py-3">
                    <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2">
                        ...
                        <span class="ml-2" data-toggle="tooltip" data-placement="top" title=""
                            data-original-title="{{translate('this_module_will_run_for_updates_after_version_13.1')}}">
                            <img class="info-img w-200" src="{{asset('/assets/back-end/img/info-circle.svg')}}"
                                alt="img">
                        </span>
                    </h5>
                </div>
                ...
                <div class="card-body">
                    <form action="{{route('admin.system-settings.software-update')}}" method="post"
                        ...
                        @php($condition_one=str_replace('M','',ini_get('upload_max_filesize'))>=180 &&
                        str_replace('M','',ini_get('upload_max_filesize'))>=180)
                        @php($condition_two=str_replace('M','',ini_get('post_max_size'))>=200 &&
                        str_replace('M','',ini_get('post_max_size'))>=200)
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="alert alert-soft-{{($condition_one)?'success':'danger'}}" role="alert">
                                    ...
                                    {{translate('value_is_greater_or_equal_to')}} 180M.
                                    {{translate('current_value_is')}} - {{ini_get('upload_max_filesize')}}
                                </div>
                                <div class="alert alert-soft-{{($condition_two)?'success':'danger'}}" role="alert">
                                    ...
                                    {{translate('value_is_greater_or_equal_to')}} 200M.
                                    {{translate('current_value_is')}}- {{ini_get('post_max_size')}}
                                </div>
                            </div>
                        </div>
                        @endif
