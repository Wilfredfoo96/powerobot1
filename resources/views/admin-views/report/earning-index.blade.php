...
<div class="content container-fluid">
                ...
                <div class="media-body">
                    <div class="row align-items-center">
                        <div class="d-block col-sm mb-1 mb-sm-0 {{Session::get('direction') === " rtl" ? 'mr-2' : 'ml-2'
                            ...
                            <div>
                                <h1 class="page-header-title">{{translate('Earning_Report')}} {{translate('overview')}}
                                </h1>
                            </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            <!-- Media -->
                            <div class="media">
                                <i class="tio-dollar-outlined nav-icon {{Session::get('direction') === " rtl"
                                    ? 'text-right' : 'text-left' }}"></i>

                                <div class="media-body {{Session::get('direction') === " rtl" ? 'text-right'
                                    : 'text-left' }}">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            <!-- Media -->
                            <div class="media">
                                <i class="tio-money nav-icon {{Session::get('direction') === " rtl" ? 'text-right'
                                    : 'text-left' }}"></i>
    ...
    <div class="card mb-3 mb-lg-5 border-top border-left border-right border-bottom">
        ...
        <div class="card-header flex-wrap">
            @php
            $total_sold=\App\Model\OrderTransaction::where(['status'=>'disburse'])->whereBetween('created_at',
            [date('y-01-01'), date('y-12-31')])->sum('order_amount');
            $t=\App\Model\OrderTransaction::where(['status'=>'disburse'])->whereBetween('created_at', [date('y-01-01'),
            date('y-12-31')])->sum('tax');
            $c=\App\Model\OrderTransaction::where(['status'=>'disburse'])->whereBetween('created_at', [date('y-01-01'),
            date('y-12-31')])->sum('admin_commission');
        ...
        for ($inc = 1; $inc <= 12; $inc++) { $sold[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) {
            $sold[$inc]=$match['sum']; } } } @endphp @php $tax=[]; $from=\Carbon\Carbon::now()->
            ...
            for ($inc = 1; $inc <= 12; $inc++) { $tax[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) {
                $tax[$inc]=$match['sum']; } } } @endphp @php $commission=[]; $from=\Carbon\Carbon::now()->
                startOfYear()->format('Y-m-d');
                $to = \Carbon\Carbon::now()->endOfYear()->format('Y-m-d');
                ...
                for ($inc = 1; $inc <= 12; $inc++) { $commission[$inc]=0; foreach ($data as $match) { if
                    ($match['month']==$inc) { $commission[$inc]=$match['sum']; } } } @endphp <!-- Body -->
                    <div class="card-body">
                        <!-- Bar Chart -->
