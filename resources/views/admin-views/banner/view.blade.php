...
<div class="content container-fluid">
        ...
        <div class="btn-group">
            ...
            <div
                ...
                <p class="title-color font-weight-medium mb-0">{{
                    translate('currently_you_are_managing_banners_for_')}}{{ucwords(str_replace("_", " ",
                    theme_root_path()))}}.{{translate('these_saved_data_is_only_applicable_only_for_')}}{{ucwords(str_replace("_",
                    " ",
                    theme_root_path()))}}.{{translate('if_you_change_theme_from_theme_setup_these_banners_will_not_be_shown_in_changed_theme._You_have_upload_all_the_banners_over_again
                    _according_to_the_new_theme_ratio_and_sizes._If_you_switch_back_to_')}}{{ucwords(str_replace("_", "
                    ", theme_root_path()))}}{{translate('_again_,_you_will_see_the_saved_data.') }}</p>
            </div>
        </div>
    </div>
    <!-- End Page Title -->
    ...
    <!-- Content Row -->
    <div class="row pb-4 d--none" id="main-banner" style="text-align: {{Session::get('direction') === " rtl" ? 'right'
        : 'left' }};">
        <div class="col-md-12">
            <div class="card">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="js-example-responsive form-control w-100" name="banner_type" required
                                        ...
                                        <option value="Main Section Banner">{{ translate('main_Section_Banner')}}
                                        </option>
                                        @endif
                                        ...
                                        <option value="Promo Banner Middle Top">{{
                                            translate('promo_banner_middle_top')}}</option>
                                        <option value="Promo Banner Middle Bottom">{{
                                            translate('promo_banner_middle_bottom')}}</option>
                                        <option value="Promo Banner Right">{{ translate('promo_banner_right')}}</option>
                                        <option value="Promo Banner Bottom">{{ translate('promo_banner_bottom')}}
                                        </option>
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="form-group mt-4 input_field_for_main_banner">
                                    <label for="button_text" class="title-color text-capitalize">{{
                                        translate('Button_Text')}}</label>
                                ...
                                <div class="form-group mt-4 mb-0 input_field_for_main_banner">
                                    <label for="background_color" class="title-color text-capitalize">{{
                                        translate('background_color')}}</label>
                            ...
                            <div class="row gy-2 gx-2 align-items-center text-left">
                                <div class="col-sm-12 col-md-9">
                                    <form action="{{ url()->current() }}" method="GET">
                                        <div class="row gy-2 gx-2 align-items-center text-left">
                                            <div class="col-sm-12 col-md-9">
                                                <select class="form-control __form-control" name="search"
                                                    id="date_type">
                                                    <option value="">{{ translate('all')}}</option>
                                                    <option value="Main Banner" {{ $search=="Main Banner" ? 'selected'
                                                        :'' }}>{{ translate('main_Banner')}}</option>
                                                    <option value="Popup Banner" {{ $search=="Popup Banner" ? 'selected'
                                                        :'' }}>{{ translate('popup_Banner')}}</option>

                                                    @if(theme_root_path() != 'theme_fashion')
                                                    <option value="Footer Banner" {{ $search=="Footer Banner"
                                                        ? 'selected' :'' }}>{{ translate('footer_Banner')}}</option>
                                                    <option value="Main Section Banner" {{
                                                        $search=="Main Section Banner" ? 'selected' :'' }}>{{
                                                        translate('main_Section_Banner')}}</option>
                                                    @endif

                                                    @if(theme_root_path() == 'theme_aster')
                                                    <option value="Header Banner" {{ $search=="Header Banner"
                                                        ? 'selected' :'' }}>{{ translate('header_Banner')}}</option>
                                                    <option value="Sidebar Banner" {{ $search=="Sidebar Banner"
                                                        ? 'selected' :'' }}>{{ translate('sidebar_Banner')}}</option>
                                                    <option value="Top Side Banner" {{ $search=="Top Side Banner"
                                                        ? 'selected' :'' }}>{{ translate('top_Side_Banner')}}</option>
                                                    @endif
                                                    @if(theme_root_path() == 'theme_fashion')
                                                    <option value="Promo Banner Left" {{ $search=="Promo Banner Left"
                                                        ? 'selected' :'' }}>{{ translate('promo_banner_left')}}</option>
                                                    <option value="Promo Banner Middle Top" {{
                                                        $search=="Promo Banner Middle Top" ? 'selected' :'' }}>{{
                                                        translate('promo_banner_middle_top')}}</option>
                                                    <option value="Promo Banner Middle Bottom" {{
                                                        $search=="Promo Banner Middle Bottom" ? 'selected' :'' }}>{{
                                                        translate('promo_banner_middle_bottom')}}</option>
                                                    <option value="Promo Banner Right" {{ $search=="Promo Banner Right"
                                                        ? 'selected' :'' }}>{{ translate('promo_banner_right')}}
                                                    </option>
                                                    <option value="Promo Banner Bottom" {{
                                                        $search=="Promo Banner Bottom" ? 'selected' :'' }}>{{
                                                        translate('promo_banner_bottom')}}</option>
                                                    @endif
                                                </select>
                                            </div>
                            ...
                            <tr id="data-{{$banner->id}}">
                                ...
                                <td>
                                    <form action="{{route('admin.banner.status')}}" method="post"
                                        id="banner_status{{$banner['id']}}_form" class="banner_status_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$banner['id']}}">
                                        <label class="switcher">
                                            <input type="checkbox" class="switcher_input"
                                                id="banner_status{{$banner['id']}}" name="status" value="1" {{
                                ...
                                <td>
                                    <div class="d-flex gap-10 justify-content-center">
                                        ...
                                        <a class="btn btn-outline-danger btn-sm cursor-pointer delete"
                                            title="{{ translate('delete')}}" id="{{$banner['id']}}">
