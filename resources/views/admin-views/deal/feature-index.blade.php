...
<div class="content container-fluid">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.deal.flash')}}" style="text-align: {{Session::get('direction') === "
                        rtl" ? 'right' : 'left' }};" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-12 {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                    id="{{$lang}}-form">
                                    <label for="name" class="title-color text-capitalize">{{ translate('title')}}
                            ...
                            <div class="row">
                                <div class="col-md-6 mt-3">
                                    <label for="name" class="title-color text-capitalize">{{
                                        translate('start_date')}}</label>
                                    <input type="date" name="start_date" required class="form-control">
                                </div>
                                <div class="col-md-6 mt-3">
                                    <label for="name" class="title-color text-capitalize">{{
                                        translate('end_date')}}</label>
                            ...
                            <form action="{{ url()->current() }}" method="GET">
                                <div class="input-group input-group-merge input-group-custom">
                                    ...
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                                        placeholder="{{translate('search by Title')}}" aria-label="Search orders"
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{route('admin.deal.feature-status')}}" method="post"
                                        id="feature_status{{$deal['id']}}_form" class="feature_status_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$deal['id']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input"
                                                id="feature_status{{$deal['id']}}" name="status" value="1" {{
                                                $deal['status']==1 ? 'checked' :'' }}
                                                onclick="toogleStatusModal(event,'feature_status{{$deal['id']}}','feature-status-on.png','feature-status-off.png','{{translate('Want_to_Turn_ON_Featured_Deal_Status')}}','{{translate('Want_to_Turn_OFF_Featured_Deal_Status')}}',`<p>{{translate('if_enabled_this_featured_deal_will_be_available_on_the_website_and_customer_app')}}</p>`,`<p>{{translate('if_disabled_this_featured_deal_will_be_hidden_from_the_website_and_customer_app')}}</p>`)">
                                ...
                                <td>
                                    <div class="d-flex align-items-center justify-content-center gap-10">
                                        ...
                                        <a title="{{ trans ('Edit')}}" href="{{route('admin.deal.edit',[$deal['id']])}}"
...
<script>
    $(document).on('change', '.featured', function () {
        ...
        $('.feature_status_form').on('submit', function(event){
