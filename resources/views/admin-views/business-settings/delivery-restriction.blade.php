...
<div class="content container-fluid">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto min-form-control-height mt-2"
                        id="customer_wallet_section">
                        <span class="title-color">
                            {{translate('delivery_available_country')}}
                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                data-placement="right"
                                title="{{translate('if_enabled_admin_can_deliver_orders_outside_his_country') }}">
                                <img width="16" src="{{asset('/assets/back-end/img/info-circle.svg')}}" alt="">
                            </span>
                        </span>

                        <form
                            action="{{ route('admin.business-settings.delivery-restriction.country-restriction-status-change') }}"
                            method="post" id="country_area_form">
                            @csrf
                            <label class="switcher">
                                <input type="checkbox" class="switcher_input" name="status" id="country_area" {{
                                    isset($country_restriction_status->value) && $country_restriction_status->value == 1
                                ? 'checked' : '' }} value="1"
                                onclick="toogleStatusModal(event,'country_area','delivery-available-country-on.png','delivery-available-country-off.png','{{translate('want_to_Turn_ON_Delivery_Available_Country')}}?','{{translate('want_to_Turn_OFF_Delivery_Available_Country')}}?',`
                                <p>{{translate('If_enabled_the_admin_or_seller_can_deliver_orders_to_the_selected_countries')}}
                                </p>`,`<p>
                                    {{translate('If_disabled_there_will_be_no_delivery_restrictions_for_admin_or_sellers')}}
                                </p>`)">
                ...
                <div class="col-md-6">
                    <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto min-form-control-height mt-2"
                        ...
                        <span class="title-color">
                            ...
                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                data-placement="right"
                                title="{{translate('if_enabled_deliveries_will_be_available_only_in_the_added_zip_code_areas') }}">
                                <img width="16" src="{{asset('/assets/back-end/img/info-circle.svg')}}" alt="">
                            </span>
                        </span>
                        ...
                        <form
                            ...
                            method="post" id="zip_area_form">
                            @csrf
                            <label class="switcher">
                                <input type="checkbox" class="switcher_input" name="status" id="zip_area" {{
                                    isset($zip_code_area_restriction_status) && $zip_code_area_restriction_status->value
                                == 1? 'checked' : '' }} value="1"
                                onclick="toogleStatusModal(event,'zip_area','zip-code-on.png','zip-code-off.png','{{translate('want_to_Turn_ON_Delivery_Available_Zip_Code_Area')}}','{{translate('want_to_Turn_OFF_Delivery_Available_Zip_Code_Area')}}',`
                                <p>{{translate('if_enabled_deliveries_will_be_available_only_in_the_added_zip_code_areas')}}
                                </p>`,`<p>
                                    {{translate('if_disabled_there_will_be_no_delivery_restrictions_based_on_zip_code_areas')}}
                                </p>`)">
                                <span class="switcher_control"></span>
                            </label>
                        </form>
    ...
    <div class="row gy-2">
        <!-- Delivery to Country -->
        <div
            class="col-lg-6 {{ isset($country_restriction_status->value) && $country_restriction_status->value  != 1 ? 'd-none' : '' }}">
            <div class="card mb-3">
                        ...
                        <div class="form-group">
                            ...
                            <div class="d-flex gap-2">
                                <select class="js-example-basic-multiple js-states js-example-responsive form-control"
                                    ...
                                    <option value="{{ $country['code'] }}" {{ in_array($country['code'],
                                        $stored_country_code) ? 'disabled' : '' }}>
                                        {{ $country['name'] }}
                                    </option>
                                    @endforeach
                                </select>
        ...
        <div
            class="col-lg-6 {{ isset($zip_code_area_restriction_status) && $zip_code_area_restriction_status->value  != 1? 'd-none' : '' }}">
            <div class="card mb-3">
