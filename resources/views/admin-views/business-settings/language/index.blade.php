...
<div class="content container-fluid">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('admin.business-settings.language.update-status') }}"
                                        method="GET" id="language_id_{{$data['id']}}_form" class="language_id_form">
                                        @csrf
                                        <input type="hidden" name="code" value="{{$data['code']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input"
                                                {{$data['status']==1?'checked':''}} id="language_id_{{$data['id']}}"
                                                name="status" class="toggle-switch-input"
                                ...
                                <td>
                                    ...
                                    <form
                                        ...
                                        method="GET" id="language_default_id_{{$data['id']}}_form"
                                        class="language_default_id_form">
                                        @csrf
                                        <input type="hidden" name="code" value="{{$data['code']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input"
                                                id="language_default_id_{{$data['id']}}" name="default"
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="recipient-name" class="col-form-label">{{translate('language')}}
                                    </label>
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control country-var-select w-100" name="code">
                                        @foreach(\Illuminate\Support\Facades\File::files(base_path('assets/front-end/img/flags'))
                                        as $path)
    ...
    @foreach(json_decode($language['value'],true) as $key =>$data)
    <div class="modal fade" id="lang-modal-update-{{$data['code']}}" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{translate('new_language')}}</h5>
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control country-var-select w-100" name="code">
                                        ...
                                        as $path)
                                        @if(pathinfo($path)['filename'] !='en' &&
                                        $data['code']==pathinfo($path)['filename'])
                                        <option value="{{ pathinfo($path)['filename'] }}"
                                            title="{{ asset('assets/front-end/img/flags/'.pathinfo($path)['filename'].'.png') }}">
                                            {{ strtoupper(pathinfo($path)['filename']) }}
                                        </option>
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    <label class="col-form-label">{{translate('direction')}} :</label>
                                    <select class="form-control" name="direction">
                                        <option value="ltr" {{isset($data['direction'])?$data['direction']=='ltr'
                                            ?'selected':'':''}}>
                                            LTR
                                        </option>
                                        <option value="rtl" {{isset($data['direction'])?$data['direction']=='rtl'
                                            ?'selected':'':''}}>
                                            RTL
                                        </option>
                                    </select>
