<div class="modal-header border-0 pb-0 d-flex justify-content-end">
    <button type="button" class="btn-close border-0" data-dismiss="modal" aria-label="Close"
        onclick="location.reload()"><i class="tio-clear"></i></button>
</div>
<div class="modal-body px-4 px-sm-5 text-center">
    ...
    <h3>
        ...
        <br>
        {{ translate('from') }} {{ ucwords(str_replace('_',' ', $current_theme)) }} {{ translate('to') }} {{
        ucwords(str_replace('_',' ', $theme_info['name'])) }}
    </h3>
    <p>
        {{ translate('please_be_reminded_that_you_have_to_setup_data_for_these_section_for') }} {{
        ucwords(str_replace('_',' ', $theme_info['name'])) }}.
    ...
    <div class="d-flex flex-column gap-2 justify-content-center align-items-center" id="notify_all_the_sellers_area">
        <button type="button" class="fs-16 btn btn--primary px-sm-5 w-fit-content" onclick="notify_all_the_sellers()">{{
            translate('notify_All_the_Sellers') }}</button>
        <button type="button" class="fs-16 btn btn-secondary px-sm-5 w-fit-content" data-dismiss="modal"
            onclick="location.reload()">{{ translate('skip') }}</button>
    </div>
</div>