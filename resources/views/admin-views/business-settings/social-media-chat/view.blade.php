...
<div class="content container-fluid">
    ...
    {{-- <div class="card mb-3">--}}
        {{-- <div class="card-body text-{{Session::get('direction') === " rtl" ? 'right' : 'left' }}">--}}
            {{-- <form--}} {{-- action="{{route('admin.social-media-chat.update',['messenger'])}}" --}} {{--
                method="post">--}}
                {{-- @csrf--}}
                ...
                {{-- <label class="switcher position-absolute right-3 top-3">--}}
                    {{-- <input class="switcher_input" type="checkbox" value="1" name="status"
                        {{$messenger['status']==1?'checked':''}}>--}}
                    {{-- <span class="switcher_control"></span>--}}
                    {{-- </label>--}}
                {{-- <div class="form-group">--}}
                    {{-- <label
                        class="title-color font-weight-bold text-capitalize">{{translate('paste_script')}}</label>--}}
...
{{--                                        </textarea>--}}
                ...
                {{-- <div class="mt-3 d-flex flex-wrap justify-content-center gap-10">--}}
                    {{-- <button type="submit"
                        class="btn btn--primary px-4 text-uppercase">{{translate('Configure')}}</button>--}}
                    {{-- </div>--}}
                {{-- @endif--}}
                {{-- </form>--}}
                {{-- </div>--}}
                ...
                <label class="switcher">
                    <input class="switcher_input" type="checkbox" value="1"
                        onclick="toogleStatusModal(event,'whatsapp_id','social/whatsapp-on.png','social/whatsapp-off.png',
                                ...
                                `<p>{{translate('if_disabled,_WhatsApp_chatting_option_will_be_hidden_from_the_system ')}}</p>`)" id="whatsapp_id" name="status"
            ...
            <div class="card-body text-start">
                ...
                <div class="form-group">
                    ...
                    <span class="ml-2" data-toggle="tooltip" data-placement="top"
                        title="{{translate('provide_a_WhatsApp_number_without_country_code')}}">
                ...
                <div class="mt-3 d-flex flex-wrap justify-content-center gap-10">
                    <button type="submit"
                        class="btn btn--primary px-5 text-uppercase">{{translate('configure')}}</button>
                </div>
                @endif
        </form>
    </div>
