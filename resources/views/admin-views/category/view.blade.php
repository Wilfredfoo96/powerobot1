                        ...
                        <ul class="nav nav-tabs w-fit-content mb-4">
                            ...
                            <li class="nav-item text-capitalize">
                                <a class="nav-link lang_link {{ $lang == $default_lang ? 'active' : '' }}" href="#"
                                    id="{{ $lang }}-link">{{ ucfirst(\App\CPU\Helpers::get_language_name($lang)) . '(' .
                                    strtoupper($lang) . ')' }}</a>
                            </li>
                            @endforeach
                        </ul>
                            ...
                            <div class="col-lg-6">
                                <div>
                                    ...
                                    <div class="form-group {{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                        ...
                                        <input type="text" name="name[]" class="form-control"
                                            placeholder="{{ translate('new_Category') }}" {{ $lang==$default_lang
                                            ? 'required' : '' }}>
                                    </div>
                                ...
                                <div class="from_part_2">
                                    ...
                                    <div class="custom-file text-left">
                                        ...
                                        <label class="custom-file-label" for="customFileEg1">{{ translate('choose_File')
                                            }}</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('admin.category.status') }}" method="post"
                                        id="category_status{{ $category['id'] }}_form" class="category_status_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $category['id'] }}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input"
                                                id="category_status{{ $category['id'] }}" name="home_status" value="1"
                                                {{ $category->home_status == 1 ? 'checked' : '' }}
                                            onclick="toogleStatusModal(event,'category_status{{ $category['id']
                                            }}','category-status-on.png','category-status-off.png','{{
                                            translate('Want_to_Turn_ON') }}
                                            {{ $category['defaultname'] }}
                                            {{ translate('Category_status') }}','{{ translate('Want_to_Turn_OFF') }}
                                            {{ $category['defaultname'] }} {{ translate('Category_status') }}',`<p>
                                                {{
                                                translate('if_enabled_this_category_it_will_be_visible_from_the_category_wise_product_section_in_the_website_and_customer_app_in_the_homepage')
                                                }}
                                            </p>`,`<p>
                                                {{
                                                translate('if_disabled_this_category_it_will_be_hidden_from_the_category_wise_product_section_in_the_website_and_customer_app_in_the_homepage')
                                                }}
                                            </p>`)">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-10">
                                        ...
                                        <a class="btn btn-outline-danger btn-sm delete square-btn"
                                            title="{{ translate('delete') }}" id="{{ $category['id'] }}">
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            toastr.success(
                            "{{ translate('category_deleted_successfully.') }}");
                            location.reload();
                        }
                    });
                }
