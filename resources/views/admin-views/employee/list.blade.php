                                ...
                                <div class="d-flex gap-2 align-items-center text-left">
                                    <div class="">
                                        <select class="form-control text-ellipsis min-w-200" name="employee_role_id">
                                            <option value="all" {{ request('employee_role')=='all' ? 'selected' : '' }}>
                                                {{translate('all')}}</option>
                                            @foreach($employee_roles as $employee_role)
                                            <option value="{{ $employee_role['id'] }}" {{
                                                request('employee_role_id')==$employee_role['id'] ? 'selected' : '' }}>
                                                {{$employee_role['name']}}
                                            </option>
                                            @endforeach
                                        </select>
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li>
                                    <a class="dropdown-item"
                                        href="{{route('admin.employee.export',['role'=>request('employee_role_id'),'search'=>request('search')])}}">
                ...
                <table id="datatable" style="text-align: {{Session::get('direction') === " rtl" ? 'right' : 'left' }};"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$e->role['name']}}</td>
                            <td>
                                <form action="{{url('/')}}/admin/employee/status" method="post"
                                    id="employee_id_{{$e['id']}}_form" class="employee_id_form">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$e['id']}}">
                                    <label class="switcher">
                                        <input type="checkbox" class="switcher_input" id="employee_id_{{$e['id']}}"
                                            name="status" class="toggle-switch-input" {{$e->status?'checked':''}}
                                        onclick="toogleStatusModal(event,'employee_id_{{$e['id']}}','employee-on.png','employee-off.png','{{translate('Want_to_Turn_ON_Employee_Status')}}','{{translate('Want_to_Turn_OFF_Employee_Status')}}',`
                                        <p>{{translate('if_enabled_this_employee_can_log_in_to_the_system_and_perform_his_role')}}
                                        </p>`,`<p>
                                            {{translate('if_disabled_this_employee_can_not_log_in_to_the_system_and_perform_his_role')}}
                                        </p>`)">
                            ...
                            <td class="text-center">
                                <div class="d-flex gap-10 justify-content-center">
                                    ...
                                    <a class="btn btn-outline-info btn-sm square-btn" title="View"
                                        href="{{route('admin.employee.view',['id'=>$e['id']])}}">
