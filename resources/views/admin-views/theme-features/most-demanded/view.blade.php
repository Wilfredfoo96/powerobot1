...
<div class="content container-fluid">
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.most-demanded.store')}}" method="post"
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div class="form-group md-2">
                                    <label for="name"
                                        class="title-color font-weight-medium">{{translate('banner')}}</label>
                                    <span class="text-info ml-1">( {{translate('ratio')}}
                                        {{translate('4')}}:{{translate('1')}} )</span>
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-4 col-lg-6 mb-2 mb-md-0">
                            <h5 class="mb-0 text-capitalize d-flex gap-2">
                                ...
                                <span class="badge badge-soft-dark radius-50 fz-12">{{ $most_demanded_products->total()
                                    }}</span>
                            </h5>
                        </div>
                    ...
                    <table id="columnSearchDatatable" style="text-align: {{Session::get('direction') === " rtl"
                        ...
                        <tbody>
                            ...
                            <tr id="data-{{$most_demanded->id}}">
                                ...
                                <td>{{ isset($most_demanded->product->name) ? $most_demanded->product->name :
                                    translate('no_product_found')}}</td>
                                <td class="d-flex justify-content-center">
                                    @if(isset($most_demanded->product->status ) && $most_demanded->product->status == 1)
                                    <form action="{{route('admin.most-demanded.status-update')}}" method="post"
                                        id="most_demanded{{$most_demanded['id']}}_form" class="most_demanded_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$most_demanded['id']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input"
                                                id="most_demanded{{$most_demanded['id']}}" name="status" value="1" {{
                                                $most_demanded['status']==1 ? 'checked' :'' }}
                                    ...
                                    <label class="switcher">
                                        <input type="checkbox" class="switcher_input status" name="status"
                                            id="{{$most_demanded->id}}" disabled>
                                ...
                                <td>
                                    <div class="d-flex gap-10 justify-content-center">
                                        ...
                                        <a class="btn btn-outline-danger btn-sm cursor-pointer delete"
                                            title="{{ translate('delete')}}" id="{{$most_demanded['id']}}">
