    ...
    <div class="row pb-4 d--none" id="main-banner" style="text-align: {{Session::get('direction') === " rtl" ? 'right'
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.business-settings.all-pages-banner-store') }}" method="post"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="js-example-responsive form-control w-100" name="type" required>
                                        ...
                                        <option value="banner_product_list_page">{{ translate('Product_List_Page')}}
                                        </option>
                                        @endif
                                        ...
                                        <option value="banner_terms_conditions">{{ translate('Terms_and_Conditions')}}
                                        </option>
                                        <option value="banner_cancellation_policy">{{ translate('Cancellation_Policy')}}
                                        </option>
                                    </select>
                                </div>
                                <div class="form-group">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-4 col-lg-6 mb-2 mb-md-0">
                            <h5 class="mb-0 text-capitalize d-flex gap-2">
                                ...
                                <span class="badge badge-soft-dark radius-50 fz-12">{{ $page_banners->total() }}</span>
                            </h5>
                        </div>
                    ...
                    <table id="columnSearchDatatable" style="text-align: {{Session::get('direction') === " rtl"
                        ...
                        <tbody>
                            <tr id="data-{{$banner->id}}">
                                ...
                                <td>
                                    <form action="{{route('admin.business-settings.all-pages-banner-status')}}"
                                        method="post" id="banner_status{{$banner['id']}}_form"
                                        class="banner_status_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$banner['id']}}">
                                        <label class="switcher">
                                            <input type="checkbox" class="switcher_input"
                                                id="banner_status{{$banner['id']}}" name="status" value="1" {{
                                                json_decode($banner['value'])->status == 1 ? 'checked':'' }}
                                            onclick="toogleStatusModal(event,'banner_status{{$banner['id']}}','banner-status-on.png','banner-status-off.png','{{translate('Want_to_Turn_ON')}}
                                            {{ucwords(translate(str_replace('_',' ',$banner->type)))}}
                                            {{translate('status')}}','{{translate('Want_to_Turn_OFF')}}
                                            {{ucwords(translate(str_replace('_',' ',$banner->type)))}}
                                            {{translate('status')}}',`<p>
                                                {{translate('if_enabled_this_banner_will_be_available_on_the_website_and_customer_app')}}
                                            </p>`,`<p>
                                                {{translate('if_disabled_this_banner_will_be_hidden_from_the_website_and_customer_app')}}
                                            </p>`)">
