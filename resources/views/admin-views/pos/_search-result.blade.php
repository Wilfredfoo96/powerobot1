...
<ul class="list-group list-unstyled gap-3">
    ...
    <li>
        <div class="media gap-3 border-bottom pb-2 cursor-pointer"
            onclick="$('.search-bar-input-mobile').val('{{$product['name']}}'); $('.search-bar-input').val('{{$product['name']}}'); quickView('{{$product->id}}')">
            ...
            <div class="media-body d-flex flex-column gap-1">
                ...
                @else
                <div class="fz-10">{{ translate('seller') }}: {{isset($product->seller) ? $product->seller->shop->name :
