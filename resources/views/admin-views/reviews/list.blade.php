                ...
                <form action="{{ url()->current() }}" method="GET">
                    <div class="input-group input-group-merge input-group-custom">
                        ...
                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                            placeholder="{{ translate('search_by_Product_or_Customer') }}" aria-label="Search orders"
        ...
        <form action="{{ url()->current() }}" method="GET">
            <div class="row gy-3 align-items-end">
                <div class="col-md-4">
                    ...
                    <div class="dropdown select-product-search w-100">
                        <input type="text" class="product_id" name="product_id" value="{{request('product_id')}}"
                            hidden>
                        <button class="form-control text-start dropdown-toggle text-truncate " data-toggle="dropdown"
                        ...
                        <div class="dropdown-menu w-100 px-2">
                            <div class="search-form mb-3">
                                ...
                                <input type="text" class="js-form-search form-control search-bar-input"
                                    onkeyup="search_product()" placeholder="{{translate('search menu')}}...">
                            </div>
                            <div
                                ...
                                <div class="select-product-item media gap-3 border-bottom pb-2 cursor-pointer">
                                    ...
                                    <div class="media-body d-flex flex-column gap-1">
                                        ...
                                        <h6 class="fz-13 mb-1 text-truncate custom-width product-name">
                                            {{$product['name']}}</h6>
                                        <div class="fz-10">{{translate('category')}} : {{isset($product->category) ?
                                            $product->category->name : translate('category_not_found') }}</div>
                                        <div class="fz-10">{{translate('brand')}} : {{isset($product->brand) ?
                                            $product->brand->name : translate('brands_not_found') }}</div>
                                        @if ($product->added_by == "seller")
                                        <div class="fz-10">{{translate('shop')}} : {{isset($product->seller) ?
                                            $product->seller->shop->name : translate('shop_not_found') }}</div>
                                        @else
                ...
                <div class="col-md-4">
                    ...
                    <select onchange="customer_id_value(this.value)" data-placeholder="@if($customer == 'all')
                                            {{translate('all_customer')}}
                                        @else
                                            {{$customer->name ? $customer->name : $customer->f_name.' '.$customer->l_name.' '.'('.$customer->phone.')'}}
                                        @endif" class="js-data-example-ajax form-control form-ellipsis">
                ...
                <div class="col-md-4">
                    <div>
                        ...
                        <input type="date" name="from" id="from_date" value="{{ $from }}" class="form-control"
                ...
                <div class="col-md-4">
                    <div>
                        ...
                        <input type="date" name="to" id="to_date" value="{{ $to }}" class="form-control"
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.product.view', [$review['product_id']]) }}"
                                ...
                                {{isset($review->product) ?Str::limit($review->product['name'], 25) :
                        ...
                        <td>
                            <div class="gap-1">
                                <div>{{ $review->comment ? Str::limit($review->comment, 35) :
                                    translate('no_comment_found') }}</div>
                                <br>
                                @if($review->attachment)
                                <div class="d-flex flex-wrap">
                                    @foreach (json_decode($review->attachment) as $img)
                                    <a href="{{ asset('storage/review') }}/{{ $img }}" data-lightbox="mygallery">
