...
@php($cart=\App\Model\Cart::where(['customer_id' => (auth('customer')->check() ? auth('customer')->id() :
session('guest_id'))])->get()->groupBy('cart_group_id'))
                ...
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2 px-0">
                    ...
                    <a href="{{route('shopView',['id'=>0])}}" class="text-primary d-flex align-items-center gap-2">
                        ...
                        <span class="pl-1 text-danger pulse-button" data-toggle="tooltip" data-placement="right"
                            ...
                            data-title="{{ translate('minimum_Order_Amount') }} {{ \App\CPU\Helpers::currency_converter($verify_status['minimum_order_amount']) }} {{ translate('for') }} @if($cartItem->seller_is=='admin') {{\App\CPU\Helpers::get_business_settings('company_name')}} @else {{ \App\CPU\get_shop_name($cartItem['seller_id']) }} @endif"
                            title="{{ translate('minimum_Order_Amount') }} {{ \App\CPU\Helpers::currency_converter($verify_status['minimum_order_amount']) }} {{ translate('for') }} @if($cartItem->seller_is=='admin') {{\App\CPU\Helpers::get_business_settings('company_name')}} @else {{ \App\CPU\get_shop_name($cartItem['seller_id']) }} @endif">
                    ...
                    <div class=" bg-white select-method-border rounded">
                        ...
                        <div class="d-sm-flex">
                            ...
                            <div class="text-sm-nowrap mx-sm-2 mt-sm-2 mb-1">
                                ...
                                :<span>{{App\CPU\Helpers::currency_converter($choosen_shipping['shipping_cost'])}}</span>
                            </div>
                            @endisset

                            <!-- chosen shipping method-->
                            <select
                                class="form-control fs-13 font-weight-bold text-capitalize border-aliceblue max-240px"
                                ...
                                <option value="{{$shipping['id']}}"
                                    ...
                                    {{translate('shipping_method')}} : {{$shipping['title'].' (
                                    '.$shipping['duration'].' )
                        ...
                        <tr>
                                ...
                                <div class="d-flex gap-3">
                                    ...
                                    <div class="d-flex flex-column gap-1">
                                        <div
                                            ...
                                            <a
                                                href="{{ $product_status == 1 ? route('product',$cartItem['slug']) : 'javascript:'}}">{{$cartItem['name']}}</a>
                                        </div>
                                        ...
                                        <div
                                            class="d-flex flex-wrap gap-2 {{ $product_status == 0?'blur-section':'' }}">
                                            <span class="fw-semibold"> {{translate('shipping_cost')}}</span>:<span>{{
                            ...
                            <td class="{{ $product_status == 0?'blur-section':'' }} __w-15p">
                                <div class="text-center">
                                    ...
                                    <span class="text-nowrap fs-10">
                                        ...
                                        ({{translate('tax')}} :
                                        {{\App\CPU\Helpers::currency_converter($cartItem['tax']*$cartItem['quantity'])}})
                                        @else
                            ...
                            <td class="__w-15p text-center">
                                ...
                                <div class="qty d-flex justify-content-center align-itmes-center gap-3">
                                    ...
                                    <input type="text" class="qty_input cartQuantity{{ $cartItem['id'] }}"
                    ...
                    <div class="d-flex align-items-center gap-8">
                        ...
                        @if ($free_delivery_status['amount_need'] <= 0) <span class="text-muted fs-12 mt-1">{{
                            ...
                            <span class="need-for-free-delivery font-bold fs-12 mt-1 text-primary">{{
                                \App\CPU\Helpers::currency_converter($free_delivery_status['amount_need']) }}</span>
            ...
            <div class="card-header d-flex flex-column gap-2 border-0 justify-content-between align-items-center">
                ...
                <div class=" bg-white select-method-border rounded">
                    ...
                    <div class="d-sm-flex">
                        ...
                        <select class="form-control fs-13 font-weight-bold text-capitalize border-aliceblue max-240px"
                            onchange="set_shipping_id(this.value,'{{$cartItem['cart_group_id']}}')">
                            <option>{{translate('choose_shipping_method')}}</option>
                            @foreach($shippings as $shipping)
                            <option value="{{$shipping['id']}}"
                                {{$choosen_shipping['shipping_method_id']==$shipping['id']?'selected':''}}>
                                {{translate('shipping_method')}} : {{$shipping['title'].' ( '.$shipping['duration'].' )
                                '.\App\CPU\Helpers::currency_converter($shipping['cost'])}}
                            </option>
                            @endforeach
                        </select>
            ...
            <div
                ...
                <div class="d-flex gap-3">
                    ...
                    <div class="d-flex flex-column gap-1">
                        ...
                        <div class="d-flex flex-wrap gap-2 {{ $product_status == 0?'blur-section':'' }}">
                            <span class="text-muted"> {{translate('shipping_cost')}}</span>:<span class="fw-semibold">{{
                                \App\CPU\Helpers::currency_converter($cartItem['shipping_cost']) }}</span>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="qty d-flex flex-column align-items-center gap-3">
                        ...
                        <input type="number" class="qty_input cartQuantity{{ $cartItem['id'] }}"
                            value="{{$cartItem['quantity']}}" name="quantity[{{ $cartItem['id'] }}]"
