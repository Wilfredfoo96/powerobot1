...
<header class="box-shadow-sm rtl __inline-10">
    ...
    <div class="topbar">
        <div class="container">
            ...
            <div>
                @php
                $currency_model = \App\CPU\Helpers::get_business_settings('currency_model');
                @endphp
                ...
                @php
                $local = \App\CPU\Helpers::default_lang();
                @endphp
    ...
    <div class="navbar-sticky bg-light mobile-head">
        <div class="navbar navbar-expand-md navbar-light">
            <div class="container ">
                ...
                <a class="navbar-brand d-none d-sm-block {{ Session::get('direction') === ' rtl' ? 'mr-3' : 'mr-3' }}
                    flex-shrink-0 __min-w-7rem" href="{{ route('home') }}">
                    <img class="__inline-11" style="height: 64px !important;"
                ...
                <div class="input-group-overlay mx-lg-4 search-form-mobile"
                    style="text-align: {{ Session::get('direction') === ' rtl' ? 'right' : 'left' }}">
                    <div style="text-align: {{ Session::get('direction') === ' rtl' ? 'left' : 'right' }}"
                        class="d-lg-none">
                    ...
                    <form action="{{ route('products') }}" type="submit" class="search_form">
                        <input class="form-control appended-form-control search-bar-input" type="text"
                            autocomplete="off" placeholder="{{ translate(' search_here') }}..." name="name">
                        <button class="input-group-append-overlay search_button" type="submit" style="border-radius: {{ Session::get('direction') === ' rtl'
                                ? '7px 0px 0px 7px; right: unset; left: 0'
                                : '0px 7px 7px 0px; left: unset; right: 0' }};top:0">
                    ...
                    <div
                        ...
                        <a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="{{ route('wishlists') }}">
                            <span class="navbar-tool-label">
                                <span class="countWishlist">{{ session()->has('wish_list') ? count(session('wish_list'))
                                    : 0 }}</span>
                            </span>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu __auth-dropdown dropdown-menu-{{ Session::get('direction') === ' rtl' ? 'left' : 'right' }}"
                            ...
                            <a class="dropdown-item" href="{{ route('customer.auth.sign-up') }}">
                                <i
                                    class="fa fa-user-circle {{ Session::get('direction') === ' rtl' ? 'mr-2' : 'mr-2' }}"></i>{{
                                translate('sign_up') }}
                            </a>
                        </div>
                    </div>
        ...
        <div class="navbar navbar-expand-md navbar-stuck-menu">
            <div class="container px-10px">
                <div class="collapse navbar-collapse" id="navbarCollapse"
                    ...
                    @endphp
                    <ul
                        class="navbar-nav mega-nav pr-lg-2 pl-lg-2 {{ Session::get('direction') === ' rtl' ? 'mr-2' : 'mr-2' }} d-none d-md-block __mega-nav">
                    ...
                    <ul class="navbar-nav mega-nav1 pr-md-2 pl-md-2 d-block d-xl-none">
                        ...
                        <li class="nav-item dropdown d-md-none">
                            <a class="nav-link dropdown-toggle {{ Session::get('direction') === ' rtl' ? 'pr-0' : 'pl-0' }}"
                                ...
                                <span
                                    style="margin-{{ Session::get('direction') === ' rtl' ? 'right' : 'left' }}: 20px;">{{
                            ...
                            <ul class="dropdown-menu __dropdown-menu-2"
                                ...
                                <li class="dropdown">
                                    ...
                                    <a <?php if ($category->childes->count() > 0) {
                                        ...
                                        href="{{ route('products', ['id' => $category['id'], 'data_from' => 'category',
                                        'page' => 1]) }}">
                    ...
                    <ul class="navbar-nav"
                        ...
                        @endif
                        @php
                        $discount_product = App\Model\Product::with(['reviews'])
                        ->active()
                        ->where('discount', '!=', 0)
                        ->count();
                        @endphp
                        ...
                        <li class="nav-item dropdown {{ request()->is('/') ? 'active' : '' }}">
                            <a class="nav-link text-capitalize"
                                ...
                                translate('discounted_products') }}</a>
                        </li>
                        @endif
                        ...
                        <li class="nav-item dropdown {{ request()->is('/') ? 'active' : '' }}">
                            <a class="nav-link text-capitalize" href="{{ route('sellers') }}">{{
                                translate('all_sellers') }}</a>
                        </li>
                        @endif
                        ...
                        @if ($business_mode == 'multi')
                        @php($seller_registration = \App\Model\BusinessSetting::where(['type' =>
                        'seller_registration'])->first()->value)
                        @if ($seller_registration)
                        <li class="nav-item">
                            <div class="dropdown">
                                <button class="btn dropdown-toggle text-white text-max-md-dark text-capitalize"
            ...
            <div class="container">
                <div class="category-menu-wrap">
                    <ul class="category-menu">
                        ...
                        <li>
                            ...
                            <div class="mega_menu z-2">
                                ...
                                <div class="mega_menu_inner">
                                    <h6><a
                                            href="{{ route('products', ['id' => $sub_category['id'], 'data_from' => 'category', 'page' => 1]) }}">{{
                                            $sub_category->name }}</a>
                                    </h6>
                                    @if ($sub_category->childes->count() > 0)
                                    @foreach ($sub_category->childes as $sub_sub_category)
                                    <div><a
                                            href="{{ route('products', ['id' => $sub_sub_category['id'], 'data_from' => 'category', 'page' => 1]) }}">{{
                        ...
                        @endforeach
                        <li class="text-center"><a href="{{ route('categories') }}"
                                class="text-primary font-weight-bold justify-content-center">View All</a></li>
                    </ul>
                </div>
            </div>
        </div>
