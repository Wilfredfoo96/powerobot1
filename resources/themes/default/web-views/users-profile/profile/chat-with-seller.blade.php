...
<style>
    .headind_srch {
        padding: {
                {
                Session: :get('direction')==="rtl" ? '10px 20px 10px 29px': '10px 29px 10px 20px'
            }
        }
    ...
    .chat_ib {
        padding: {
                {
                Session: :get('direction')==="rtl" ? '0 15px 6px 0': '0 0 6px 15px'
            }
        }
    ...
    .received_msg {
        padding: {
                {
                Session: :get('direction')==="rtl" ? '0 10px 0 0': '0 0 0 10px'
            }
        }
    ...
    .received_withd_msg p {
        padding: {
                {
                Session: :get('direction')==="rtl" ? '4px 10px 3px 8px': '4px 8px 3px 10px'
            }
        }
    ...
    .send_msg p {
        padding: {
                {
                Session: :get('direction')==="rtl" ? '5px 12px 5px 10px': '5px 10px 5px 12px'
            }
        }
                        ...
                        <div class="chat--sidebar-card h-100">
                            <div class="chat--sidebar-top">
                                ...
                                <div class="headind_srch px-0">
                                    <form
                                        class="rounded bg-white form-inline d-flex justify-content-center md-form form-sm active-cyan-2 mt-2">
                                        <input
                                            class="form-control form-control-sm border-0 {{Session::get('direction') === "
                                            rtl" ? 'ml-3' : 'mr-3' }} w-75" id="myInput" type="text"
                            ...
                            <div class="inbox_chat">
                                ...
                                <div class="chat_list {{($last_chat->delivery_man_id==$unique_id || $last_chat->shop_id==$unique_id) ? 'active' : ''}}"
                                            ...
                                            <div>
                                                <div
                                                    ...
                                                    <h5 class="{{$shop->seen_by_customer == 0 ? 'active-text' : ''}}">
                                                        {{$shop->f_name? $shop->f_name. ' ' . $shop->l_name:
                                                        $shop->name}}</h5>
                                                    <span class="date">
                                                        @if ($shop->created_at->diffInDays(\Carbon\Carbon::now()) < 1)
                                                            <?php $createdAt=\Carbon\Carbon::parse($shop->created_at);
                                                            $currentDateTime = \Carbon\Carbon::now();
                                                            ?>
                                                            <small class="date">{{
                                                                $createdAt->diffInHours($currentDateTime) > 0 ?
                                                                $createdAt->diffInHours($currentDateTime) . ' ' .
                                                                translate('hour') :
                                                                $createdAt->diffInMinutes($currentDateTime) . ' ' .
                                                                translate('minutes') }} {{ translate('ago') }}</small>
                                                            @elseif
                                                            ($shop->created_at->diffInDays(\Carbon\Carbon::now()) < 7)
                                                                <small class="date">{{ date('D
                                                                h:i:A',strtotime($shop->created_at)) }}</small>
                                                                @else
                                                                <small class="date">{{ date('d M Y
                                                                    h:i:A',strtotime($shop->created_at)) }}</small>
                                                                @endif
                                                    </span>
                                                </div>
                                                <div
                                                    ...
                                                    <span
                                                        class="new-msg badge btn--primary rounded-full">{{$shop->unseen_message_count
                                                        >0 ? $shop->unseen_message_count : ''}}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="mesgs">
                                            <a class="msg-user" href="#">
                                                <img src="{{ $last_chat->delivery_man ?asset('storage/delivery-man/'.$last_chat->delivery_man->image) : asset('storage/shop/'.$last_chat->shop->image)}}"
                                                    ...
                                                    alt="" class="img">
                                                <h5 class="m-0">
                                                    {{$last_chat->delivery_man?$last_chat->delivery_man->f_name.'
                                                    '.$last_chat->delivery_man->l_name : $last_chat->shop->name }}</h5>
                                            </a>
                                            <div class="msg_history" id="show_msg">
                                                @if (isset($chattings))
                                                @foreach($chattings as $key => $chat)
                                                @if ($chat->sent_by_seller? $chat->sent_by_seller :
                                                $chat->sent_by_delivery_man)
                                                <div class="outgoing_msg incoming_msg">
                                                    <div class="incoming_msg_img"><img
                                                            src="{{ $shop->delivery_man_id ?asset('storage/delivery-man/'.$last_chat->delivery_man->image) : asset('storage/shop/'.$last_chat->shop->image)}}"
                                                            onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'"
                                                            alt="photo"></div>
                                                    <div class="received_msg">
                                                        <div class="received_withd_msg">
                                                            ...
                                                            <span class="time_date"> {{$chat->created_at->format('h:i
                                                                A')}} | {{$chat->created_at->format('M d')}} </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                @else
                                                <div class="outgoing_msg">
                                                    <div class="send_msg">
                                                        ...
                                                        <div class="d-flex gap-3 flex-wrap mt-3">
                                                            @foreach (json_decode($chat['attachment']) as $key =>
                                                            $photo)
                                                            <div
                                                                class="w-70px spartan_item_wrapper position-relative img_row{{$key}}">
                                                        ...
                                                        @endif
                                                        <span class="time_date"> {{$chat->created_at->format('h:i A')}}
                                            ...
                                            <div class="type_msg">
                                                <div class="input_msg_write">
                                                    <form
                                                        class="d-flex justify-content-center align-items-center md-form form-sm active-cyan-2"
                                                        id="myForm" enctype="multipart/form-data">
                                                        @csrf
                                                        <label
                                                            class="py-0 px-3 d-flex align-items-center m-0 cursor-pointer">
                                                            ...
                                                            <input type="file" name="image[]" id="f_p_v_up1"
                                                        ...
                                                        <input type="text" id="hidden_value_dm" hidden
                                                            value="{{$last_chat->delivery_man_id}}"
                                                        ...
                                                        <div class="w-0 flex-grow-1">
                                                            <textarea class="form-control ticket-view-control px-0 py-3"
                                                                name="comment" rows="8" id="msgInputValue"
                        ...
                        respons.image.forEach(function (imageUrl, index) {
                            ...
                            imageContainer += `
                                <div class="w-70px spartan_item_wrapper position-relative img_row${index}">
                                    <img class="border rounded border-primary-light h-70px" onerror="this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'"
