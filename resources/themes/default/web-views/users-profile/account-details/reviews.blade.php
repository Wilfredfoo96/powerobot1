                    ...
                    <div class="border rounded bg-white p-3">
                        <div class="media gap-3">
                            <div class="position-relative">
                                <img class="d-block review-item-img"
                                    onclick="location.href='{{route('product',$order_details->product['slug'])}}'"
                            ...
                            <div class="media-body">
                                ...
                                <div><small class="text-muted">{{translate('variant')}} :
                                        {{$order_details->variant}}</small></div>
                                @endif
                                <br>
                                <div class="d-flex justify-content-end justify-content-sm-start">
                                    <button type="button" class="btn btn-sm rounded btn-warning" data-toggle="modal"
                                        data-target="#submitReviewModal{{$order_details->id}}"><i
                                            class="tio-star-half"></i>{{translate('update_review')}}</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            <div class="d-flex justify-content-between gap-2 mb-3">
                                <h6 class="d-flex gap-2 mb-0 review-item-title">
                                    <span>{{number_format($order_details->product->reviews_by_customer[0]->rating
                                        ,1)}}<i class="tio-star-half text-warning text-capitalize"></i></span>
                                    {{translate('my_review')}}
                                </h6>
                                <div class="text-muted fs-12">{{isset($order_details->product->reviews_by_customer[0]) ?
                                    date('M d , Y h:i
                                    A',strtotime($order_details->product->reviews_by_customer[0]->updated_at)) : ''}}
                                </div>
                            </div>
                            <p class="fs-12">{{isset($order_details->product->reviews_by_customer[0]) ?
                                $order_details->product->reviews_by_customer[0]->comment:''}}</p>
                            @if (isset($order_details->product->reviews_by_customer[0]) &&
                            $order_details->product->reviews_by_customer[0]->attachment &&
                            $order_details->product->reviews_by_customer[0]->attachment != [])
                            @if(json_decode($order_details->product->reviews_by_customer[0]->attachment) != [])
                            <div class="mt-3">
                                <div class="d-flex gap-2 flex-wrap">
                                    @foreach (json_decode($order_details->product->reviews_by_customer[0]->attachment)
                                    as $key => $photo)
                                    <a data-lightbox="mygallery" href="{{asset('storage/review')}}/{{$photo}}">
                                        <img class="border rounded border-primary-light"
                                            onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'"
                                            src="{{asset('storage/review')}}/{{$photo}}" alt="VR Collection" width="60">
                                    </a>
                                    @endforeach
                                </div>
                            </div>
