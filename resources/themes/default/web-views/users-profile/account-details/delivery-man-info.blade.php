            ...
            <div class="bg-sm-white mt-3">
                <div class="p-sm-3">
                    ...
                    <div class="delivery-man-info-box bg-white media gap-2 gap-sm-3 shadow-sm rounded p-3">
                        <img class="rounded-circle" width="77"
                            ...
                            <div
                                ...
                                <div class="">
                                    <h6 class="text-capitalize mb-2">
                                        {{$order->delivery_man->f_name}}&nbsp{{$order->delivery_man->l_name}}</h6>
                                    <div class="rating-show justify-content-between fs-12">
                                        <span class="d-inline-block text-body">
                                            @php($avg_rating = isset($order->delivery_man->rating[0]->average) ?
                                            $order->delivery_man->rating[0]->average : 0)
                                            @for($inc=1;$inc<=5;$inc++) @if ($inc <=(int)$avg_rating) <i
                                                class="tio-star text-warning"></i>
                                                @elseif ($avg_rating != 0 && $inc <= (int)$avg_rating + 1.1 &&
                                                    ...
                                                    @endif
                                                    @endfor
                                                    <label class="badge-style fs-12">( {{number_format($avg_rating,1)}}
                                                        )</label>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-end flex-wrap gap-3 gap-sm-3">
                                    ...
                                    <button type="button" class="btn btn-sm btn-warning px-2 px-md-4"
                                        data-toggle="modal" data-target="#submitReviewModal">
                                        <i class="tio-star-half"></i>
                                        @if(isset($order->delivery_man_review->comment) ||
                                        isset($order->delivery_man_review->rating))
                    ...
                    <div class="shadow-sm bg-white rounded p-3 mt-3">
                        <div class="d-flex align-items-center flex-wrap justify-content-between gap-2 mb-3">
                            <h6 class="d-flex gap-2 mb-0 review-item-title">
                                <span>{{number_format($order->delivery_man_review?->rating ,1)}}<i
                                        class="tio-star-half text-warning text-capitalize"></i></span>
                                {{translate('delivery_man_review')}}
                            </h6>
                            <div class="fs-12 text-muted">{{date('M d , Y h:i
                                A',strtotime($order->delivery_man_review->updated_at))}}</div>
                        </div>
                    ...
                    <div class="shadow-sm rounded bg-white p-3 mt-3">
                        <h6 class="mb-0 fs-12 d-flex align-items-center gap-2 lh-1 mb-3">
                            ...
                            {{translate('picture_upload_by')}}
                            {{$order->delivery_man->f_name}}&nbsp{{$order->delivery_man->l_name}}
                        </h6>
                            ...
                            <div class="col-sm-6 col-xl-4">
                                <div class="media gap-3">
                                    <img onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'"
                                        src="{{asset('assets/front-end/img/icons/van.png')}}" alt="VR Collection"
                                        width="20">
                                    <div class="media-body">
                                        <div class="text-muted text-capitalize">{{translate('delivery_service_name')}}
                                        </div>
                            ...
                            <div class="col-sm-6 col-xl-4">
                                <div class="media gap-3">
                                    ...
                                    <div class="media-body">
                                        ...
                                        <div class="font-weight-bold">{{$order->third_party_delivery_tracking_id ?? ''}}
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <form action="{{route('submit-deliveryman-review')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="d-flex flex-column gap-2 align-items-center my-4">
                        ...
                        <div class="rating-label-wrap position-relative">
                            <label class="rating-label">
                                <input name="rating" class="rating" max="5" min="1"
                    ...
                    <div class="">
                        <textarea rows="4" name="comment" class="form-control text-area-class"
                            placeholder="{{translate('best_delivery_service,_highly_recommended')}}">{{$order->delivery_man_review->comment ?? ''}}</textarea>
                    </div>
