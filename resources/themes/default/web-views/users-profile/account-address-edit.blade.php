...
@section('content')
<div class="container py-4 rtl __account-address" style="text-align: {{Session::get('direction') === " rtl" ? 'right'
    : 'left' }};">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('address-update')}}" method="post">
                                ...
                                <ul class="donate-now">
                                    <li class="address_type_li">
                                        <input type="radio" class="address_type" id="a25" name="addressAs"
                                            value="permanent" {{ $shippingAddress->address_type == 'permanent' ?
                                        'checked' : ''}} />
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="zip" class="form-control selectpicker" data-live-search="true" id=""
                                    ...
                                    <option value="{{ $zip->zipcode }}" {{ $zip->zipcode == $shippingAddress->zip?
                                        'selected' : ''}}>{{ $zip->zipcode }}</option>
                                    @endforeach
                                </select>
                                @else
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                ...
                                <select name="country" class="form-control selectpicker" data-live-search="true" id=""
                                    ...
                                    <option value="{{$country['name']}}" {{ $country['name']==$shippingAddress->country?
                                        'selected' : ''}}>{{$country['name']}}</option>
                                    @endforeach
                                    @else
                                    @foreach(COUNTRIES as $country)
                                    <option value="{{ $country['name'] }}" {{ $shippingAddress->country ==
                        ...
                        <input type="hidden" id="latitude" name="latitude" class="form-control d-inline"
                            placeholder="{{ translate('ex')}} : -94.22213" value="{{$shipping_latitude??0}}" required
                            readonly>
