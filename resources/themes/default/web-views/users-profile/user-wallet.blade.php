...
@section('content')
<!-- Page Content-->
<div class="container pb-5 mb-2 mb-md-4 mt-3 rtl" style="text-align: {{Session::get('direction') === " rtl" ? 'right'
    : 'left' }};">
    <div class="row">
        <!-- Sidebar-->
                                    ...
                                    <div class="card btn--primary h-100 position-relative mx-h-200">
                                        <div class="card-body d-flex align-items-center z-2">
                                            <div
                                                ...
                                                <div class="mx-3">
                                                    <button class="btn btn-light align-items-center" data-toggle="modal"
                                                        data-target="#addFundToWallet">
                                        ...
                                        <img class="wallet-card-bg z-1"
                                            src="{{ asset('assets/front-end/img/icons/wallet-card.png') }}" alt="">
                                    </div>
                                </div>
                                <div class="col-md-7">
                                    @if($add_funds_to_wallet)
                                        ...
                                        <div class="item">
                                            ...
                                            <div class="add-fund-carousel-card z-1 w-100 border rounded-10 p-4 ml-1">
                                                <div>
                                                    ...
                                                    <p class="mb-2 text-dark">{{ translate('valid_till') }} {{ date('d
                                                        M, Y',strtotime($bonus->end_date_time)) }}</p>
                                                </div>
                                                <div>
                                                    @if ($bonus->bonus_type == 'percentage')
                                                    <p>{{ translate('add_fund_to_wallet') }} {{
                                                        \App\CPU\Helpers::currency_converter($bonus->min_add_money_amount)
                                                        }} {{ translate('and_enjoy') }} {{ $bonus->bonus_amount }}% {{
                                                        translate('bonus') }}</p>
                                                    @else
                                                    <p>{{ translate('add_fund_to_wallet') }} {{
                                                        \App\CPU\Helpers::currency_converter($bonus->min_add_money_amount)
                                                        }} {{ translate('and_enjoy') }} {{
                                                        \App\CPU\Helpers::currency_converter($bonus->bonus_amount) }} {{
                                                        translate('bonus') }}</p>
                                                    @endif
                                                    <p class="fw-bold text-accent mb-0">{{ $bonus->description ?
                                                        Str::limit($bonus->description, 40):'' }}</p>
                                                </div>
                                                <img class="slider-card-bg-img" width="50"
                                                    src="{{ asset('assets/front-end/img/icons/add_fund_vector.png') }}"
                                                    alt="">
                                            </div>
                                        </div>
                                        @endforeach
                                    ...
                                    <div class="modal-body px-5">
                                        ...
                                        <form action="{{ route('customer.add-fund-request') }}" method="post">
                                            ...
                                            <div class="pb-4">
                                                ...
                                                <input type="number"
                                                    class="h-70 form-control text-center text-24 rounded-10"
                                                    id="add-fund-amount-input" name="amount" required
                                                    placeholder="{{ \App\CPU\currency_symbol() }}500">
                                            ...
                                            <div id="add-fund-list-area" style="display: none">
                                                <h6 class="mb-2">{{ translate('payment_Methods') }} <small>({{
                                                        translate('faster_&_secure_way_to_pay_bill') }})</small></h6>
                                            ...
                                            <div class="d-flex justify-content-center pt-2 pb-3">
                                                <button type="submit" class="btn btn--primary w-75 mx-3"
                                                    id="add_fund_to_wallet_form_btn">{{ translate('add_Fund')
                                                    }}</button>
                                            </div>
                                        </form>
                        ...
                        <div class="col-md-12">
                            <div class="">
                                <div
                                    ...
                                    <ul class="navbar-nav text-center" style="{{Session::get('direction') === " rtl"
                                        ? 'padding-right: 0px' : '' }}">
                                        <div class="dropdown border pl-3">
                                            <button class="btn btn-sm dropdown-toggle" type="button"
                                                id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true"
                                                aria-expanded="false" style="padding-{{Session::get('direction') === "
                                                rtl" ? 'right' : 'left' }}: 0">
                                                {{ request()->has('type') ? (request('type') == 'all'?
                                                translate('all_Transactions') :
                                                ucwords(translate(request('type')))):translate('all_Transactions')}}
                                            </button>

                                            <div class="dropdown-menu __dropdown-menu-3 __min-w-165px"
                                                aria-labelledby="dropdownMenuButton"
                                                style="text-align: {{Session::get('direction') === " rtl" ? 'right'
                                                : 'left' }};transform: translate3d(7px, -230px, 0) !important;">
                                        ...
                                        <div class="bg-light my-1 p-3 rounded d-flex justify-content-between g-2">
                                            <div class="w-100">
                                                <h6 class="mb-2 d-flex align-items-center gap-8">
                                                    @if($item['debit'] != 0)
                                                    <img src="{{ asset('assets/front-end/img/icons/coin-danger.png') }}"
                                                        width="25" alt="">
                                                    @else
                                                    <img src="{{ asset('assets/front-end/img/icons/coin-success.png') }}"
                                                        width="25" alt="">
                                                    @endif

                                                    {{ $item['debit'] != 0 ? ' -
                                                    '.\App\CPU\Helpers::currency_converter($item['debit']) : ' +
                                                    '.\App\CPU\Helpers::currency_converter($item['credit']) }}

                                                </h6>
                                                <h6 class="text-muted mb-0 small">
                                                    @if ($item['transaction_type'] == 'add_fund_by_admin')
                                                    {{translate('add_fund_by_admin')}} {{ $item['reference']
                                                    =='earned_by_referral' ? '('.translate($item['reference']).')' : ''
                                                    }}
                                            ...
                                            <div class="text-end small">
                                                <div class="text-muted mb-1 text-nowrap">{{date('d M, Y H:i
                                                    A',strtotime($item['created_at']))}}</div>
                                                @if($item['debit'] != 0)
                                ...
                                <div class="d-flex flex-column gap-3 align-items-center text-center my-5">
                                    <img width="72"
                                        src="{{ asset('assets/front-end/img/icons/empty-transaction-history.png')}}"
                                ...
                                <div class="card-footer bg-transparent border-0 p-0 mt-3">
                                    ...
                                    @php($modifiedLinks = preg_replace('/href="([^"]*)"/',
                                    'href="$1&type='.request('type').'"', $paginationLinks))
                                    @else
