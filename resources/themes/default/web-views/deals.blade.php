...
<div class="__inline-59 pt-md-3">
    ...
    @endif
    <div class="container md-4 mt-3 rtl" style="text-align: {{Session::get('direction') === " rtl" ? 'right' : 'left'
        }};">
        <div class="__flash-deals-bg" style="background: url({{$deal_banner}}) no-repeat center center / cover">
            <div class="row g-3 flex-center align-items-center">
                ...
                <=',date('Y-m-d'))->whereDate('end_date','>=',date('Y-m-d'))->first())
                    <div class="col-lg-4 col-md-6 text-center {{Session::get('direction') === " rtl" ? 'text-sm-right'
                        : 'text-sm-left' }}">
                        <div class="flash_deal_title text-base">
                            {{$web_config['flash_deals']->title}}
                        </div>
                        <span class="text-base">{{translate('hurry_Up')}} ! {{translate('the_offer_is_limited')}}.
                            {{translate('grab_while_it_lasts')}}</span>
                    </div>
