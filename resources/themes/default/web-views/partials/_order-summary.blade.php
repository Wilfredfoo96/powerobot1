    ...
    <div class="__cart-total __cart-total_sticky">
        <div class="cart_total p-0">
            ...
            <h6 class="text-center text-primary mb-4 d-flex align-items-center justify-content-center gap-2">
                ...
                <strong>{{\App\CPU\Helpers::currency_converter($total_discount_on_product)}}!</strong>
            </h6>
            @endif
                ...
                <form class="needs-validation coupon-code-form" action="javascript:" method="post" novalidate
                    ...
                    <div class="d-flex form-control rounded-pill pl-3 p-1">
                        ...
                        <button class="btn btn--primary rounded-pill text-uppercase py-1 fs-12" type="button"
                            onclick="couponCode()">{{translate('apply')}}</button>
                    </div>
        ...
        <div class="mt-5">
            <div class="row justify-content-center g-4">
                ...
                <div class="col-sm-3 px-0 text-center mobile-padding">
                    <img class="order-summery-footer-image" src="{{asset("
                        /storage/company-reliability").'/'.$value['image']}}"
        ...
        <div class="mt-4">
            ...
            <a onclick="checkout()"
                class="btn btn--primary btn-block proceed_to_next_button {{$cart->count() <= 0 ? 'disabled' : ''}}">{{translate('proceed_to_Next')}}</a>
            @else
...
<div class="bottom-sticky3 bg-white p-3 shadow-sm w-100 d-lg-none">
    <div class="d-flex justify-content-center align-items-center fs-14 mb-2">
        ...
        &nbsp; <strong
            class="text-base">{{\App\CPU\Helpers::currency_converter($sub_total+$total_tax+$total_shipping_cost-$coupon_dis-$total_discount_on_product-$order_wise_shipping_discount)}}</strong>
    </div>
    @if($web_config['guest_checkout_status'] || auth('customer')->check())
    <a onclick="checkout()"
        class="btn btn--primary btn-block proceed_to_next_button text-capitalize {{$cart->count() <= 0 ? 'disabled' : ''}}">{{translate('proceed_to_next')}}</a>
    @else
    <a href="{{route('customer.auth.login')}}"
        class="btn btn--primary btn-block proceed_to_next_button text-capitalize {{$cart->count() <= 0 ? 'disabled' : ''}}">{{translate('proceed_to_next')}}</a>
    @endif
