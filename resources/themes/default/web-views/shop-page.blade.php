...
<div class="container py-4 __inline-67">
    <div class="rtl">
        ...
        <div class="bg-white __shop-banner-main">
            ...
            @endif
            <!-- seller info+contact -->
            <div class="position-relatve z-index-99 rtl w-100" style="text-align: {{Session::get('direction') === " rtl"
                ? 'right' : 'left' }};">
                <div class="__rounded-10 bg-white position-relative">
                    <div class="d-flex flex-wrap justify-content-between seller-details">
                        ...
                        <div class="d-flex align-items-center p-2 flex-grow-1">
                                ...
                                <div class="position-relative">
                                    ...
                                    $inhouse_vacation_start_date && $current_date <= $inhouse_vacation_end_date) ||
                                        ...
                                        $seller_vacation_start_date && $current_date <= $seller_vacation_end_date) <span
                                            class="temporary-closed-details">
                                            <span>{{translate('closed_now')}}</span>
                                            </span>
                                            @endif
                            ...
                            <div class="__w-100px flex-grow-1 {{Session::get('direction') === " rtl" ? ' pr-2 pr-sm-4'
                                : ' pl-2 pl-sm-4' }}">
                                ...
                                <div class="">
                                    <div class="d-flex flex-wrap __text-12px py-1 fw-bold"
                                        ...
                                        <span>{{ \App\CPU\Helpers::currency_converter($minimum_order_amount)}}
                                            {{translate('minimum_order_amount')}}</span>
                                        @else
                                        <span>{{
                                            \App\CPU\Helpers::currency_converter($shop->seller->minimum_order_amount)}}
                                            {{translate('minimum_order_amount')}}</span>
                                        @endif
                                        @endif
                                    </div>
                                    <div>
                                        @for ($i = 1; $i <= 5; $i++) @if ($i <=$avg_rating) <i
                        ...
                        <div class="d-flex align-items-center">
                            <div class="{{Session::get('direction') === " rtl" ? 'ml-sm-4' : 'mr-sm-4' }}">
                                ...
                                <div class="d-flex">
                                    <button
                                        class="btn btn--primary __inline-70 rounded-10  text-capitalize chat-with-seller-button d-none d-sm-inline-block"
                                        data-toggle="modal" data-target="#exampleModal" {{ ($shop->temporary_close ||
                                        ($shop->vacation_status && date('Y-m-d') >= date('Y-m-d',
                                        strtotime($shop->vacation_start_date)) && date('Y-m-d') <= date('Y-m-d',
                                            strtotime($shop->vacation_end_date)))) ? 'disabled' : '' }}>
                                            <img src="{{asset('/assets/front-end/img/shopview-chat.png')}}"
                                    ...
                                    <button
                                        ...
                                        strtotime($shop->vacation_start_date)) && date('Y-m-d') <= date('Y-m-d',
                                            strtotime($shop->vacation_end_date)))) ? 'disabled' : '' }}>
                                            <img src="{{asset('/assets/front-end/img/icons/shopview-chat-blue.svg')}}"
                                                loading="eager" class="" alt="">
                                    </button>
                                ...
                                <div class="d-flex">
                                    <a href="{{route('customer.auth.login')}}"
                                        ...
                                        <img src="{{asset('/assets/front-end/img/shopview-chat.png')}}" loading="eager"
                                            class="" alt="">
                                        <span class="d-none d-sm-inline-block">
                                            {{translate('chat_with_seller')}}
                                        </span>
    ...
    <div class="d-flex flex-wrap gap-3 justify-content-sm-between py-4" dir="{{Session::get('direction') === " rtl"
        ? 'right' : 'left' }}">
        ...
        <div class="d-flex flex-column flex-sm-row gap-3">
            ...
            <form method="get" action="{{route('shopView',['id'=>$seller_id])}}">
                <div class="search_form input-group search-form-input-group">
                    ...
                    <input type="text" class="form-control rounded-left" name="product_name"
                                    ...
                                    <div class="for-hover-lable card-header flex-between">
                                        <div>
                                            <label class="cursor-pointer"
                                                onclick="location.href='{{route('shopView',['id'=> $seller_id,'sub_category_id'=>$child['id']])}}'">
                                                {{$child['name']}}
                                            </label>
                                        </div>
                                        <div class="px-2 cursor-pointer menu--caret">
                                    ...
                                    <div class="card-body p-0 {{Session::get('direction') === " rtl" ? 'mr-2' : 'ml-2'
                                        ...
                                        <div class="card-header">
                                            <label class="for-hover-lable d-block cursor-pointer text-left"
                                                onclick="location.href='{{route('shopView',['id'=> $seller_id,'sub_sub_category_id'=>$ch['id']])}}'">
                                                {{$ch['name']}}
                                            </label>
                                        </div>
                                        @endforeach
