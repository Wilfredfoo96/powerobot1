...
<div class="container mb-md-4 {{Session::get('direction') === " rtl" ? 'rtl' : '' }} __inline-65">
    <div class="bg-primary-light rounded-10 my-4 p-3 p-sm-4"
        data-bg-img="{{ asset('assets/front-end/img/media/bg.png') }}">
        <div class="row g-2 align-items-center">
            <div class="col-lg-8 col-md-6">
                <div class="d-flex flex-column gap-1 text-primary">
                    ...
                    <p class="fs-14 fw-semibold mb-0">{{translate('Find your desired stores and shop your favourite
                        products')}}</p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6">
                <form action="{{route('search-shop')}}">
                    <div class="input-group">
                        <input type="text" class="form-control rounded-10" value="{{request('shop_name')}}"
                            placeholder="{{translate('Search_Store')}}" name="shop_name">
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary rounded-10"
                    ...
                    <a href="{{route('shopView',['id'=>$seller['id']])}}" class="others-store-card text-capitalize">
                        ...
                        <div class="name-area">
                            <div class="position-relative">
                                ...
                                @if($seller->temporary_close || ($seller->vacation_status && ($current_date >=
                                $seller->vacation_start_date) && ($current_date <= $seller->vacation_end_date)))
                        ...
                        <div class="info-area">
                            <div class="info-item">
                                <h6 style="color:{{$web_config['primary_color']}}">{{$seller->review_count < 1000 ?
                                        $seller->review_count : number_format($seller->review_count/1000 , 1).'K'}}</h6>
                                <span>{{ translate('reviews') }}</span>
                            </div>
                            <div class="info-item">
                                <h6 style="color:{{$web_config['primary_color']}}">{{$seller->product_count < 1000 ?
                                        $seller->product_count : number_format($seller->product_count/1000 , 1).'K'}}
                                </h6>
