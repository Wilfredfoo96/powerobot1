                                            ...
                                            <div class="media align-items-start justify-content-between">
                                                <div class="media-body">
                                                    ...
                                                    <div class="fs-12 text-muted mb-1">{{
                                                        \App\CPU\customer_info()->email }}</div>
                                                </div>
                                                @if($supportTicket->status != 'close')
                                                <a href="{{route('support-ticket.close',[$supportTicket['id']])}}"
                                                    class="btn btn-outline-danger fw-semibold text-nowrap">{{
                                                    translate('Close_ticket') }}</a>
                                                @endif
                                            </div>

                                            <div class="d-flex flex-wrap align-items-center gap-2 gap-sm-3">
                                                <span @if($supportTicket->priority == 'Urgent')
                                                ...
                                                <span
                                                    class="{{$supportTicket->status ==  'open' ? 'badge bg-info' : 'badge bg-danger'}} rounded-pill">{{
                                                    translate($supportTicket->status) }}</span>
                                                <span class="badge bg-white text-dark">{{
                                                    translate($supportTicket->type) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
