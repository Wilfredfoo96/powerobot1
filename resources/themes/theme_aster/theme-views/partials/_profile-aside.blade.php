    ...
    <div class="card profile-sidebar-sticky">
        <div class="card-body position-relative">
            <div class="d-none d-lg-flex justify-content-end">
                <div class="dropdown">
                    <button class="btn-circle p-0 bg-primary absolute-white" type="button" data-bs-toggle="dropdown"
                        aria-expanded="false" style="--size: 1.125rem">
                        <i class="bi bi-three-dots fs-10 grid-center"></i>
                    </button>
                    <ul class="dropdown-menu">
                        <li>
                            <a href="javascript:"
                                onclick="route_alert('{{ route('account-delete',[$customer_info['id']]) }}','{{ translate('want_to_delete_this_account?')}}')">
            ...
            <div class="d-flex flex-row flex-lg-column gap-2 gap-lg-4 align-items-center">
                <div class="avatar overflow-hidden profile-sidebar-avatar border border-primary rounded-circle p-1">
                    <img onerror="this.src='{{ theme_asset('assets/img/image-place-holder.png') }}'"
                        src="{{asset('storage/profile')}}/{{$customer_info->image}}" alt=""
                        class="img-fit dark-support rounded-circle">
                </div>

                <div class="text-lg-center">
                    <h5 class="mb-1">{{$customer_info->f_name}} {{$customer_info->l_name}}</h5>
                    <p class="fw-medium">{{translate('Joined')}} {{date('d M,
                        Y',strtotime($customer_info->created_at))}}</p>
                </div>
            </div>
