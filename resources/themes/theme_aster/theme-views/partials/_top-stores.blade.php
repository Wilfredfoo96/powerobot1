                                ...
                                <div class="swiper-slide align-items-start bg-light rounded">
                                    ...
                                    <div class="bg-light position-relative rounded p-3 p-sm-4 w-100">
                                        ...
                                        <div class="offer-text">
                                            {{ translate('USE_COUPON') }} : <span class="cursor-pointer"
                                                onclick="coupon_copy('{{ $seller->coupon[0]['code'] }}')">{{
                                                $seller->coupon[0]['code'] }}</span>
                                        </div>
                                        @endif

                                        <div class="{{ count($seller->coupon)>0 ? 'mt-4' :'' }} mb-3">
                                            <h5 class="mb-1"><a href="javascript:"
                                                    onclick="location.href='{{route('shopView',['id'=>$seller['id']])}}'">{{
                                                    $seller->shop->name }}</a></h5>
                                            <div class="text-muted">{{ $seller->product_count }} {{
                                                translate('products') }}</div>
                                            <div class="d-flex gap-2 align-items-center mt-1">
                                                <div class="star-rating text-gold fs-12">
                                                    @for($inc=0;$inc<5;$inc++) @if($inc<$seller->average_rating)
                                        ...
                                        @if($seller->product)
                                        <div class="auto-col gap-3"
                                            style="--minWidth: 3.75rem; --maxWidth: {{ count($seller->product)==1 ? '6.5rem' : '1fr' }}">
                                            ...
                                            <a href="{{route('product',$product['slug'])}}"
                                                ...
                                                <div class="store-product__top border rounded">
                                                    <span class="store-product__action preventDefault">
                                                        <i class="bi bi-eye fs-12"
                                                            onclick="quickView('{{$product['id']}}', '{{route('quick-view')}}')"></i>
                                                    </span>
