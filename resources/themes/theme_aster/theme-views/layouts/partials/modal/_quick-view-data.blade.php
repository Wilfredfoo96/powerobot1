            ...
            <div class="row align-items-center gy-4">
                                ...
                                <div class="swiper-slide position-relative" id="preview-box-{{ $photo->color }}">
                                    <div class="easyzoom easyzoom--overlay">
                                        ...
                                        <a href="{{asset(" storage/product/".$photo->image_name)}}">
                                            <img src="{{asset(" storage/product/".$photo->image_name)}}"
                            ...
                            <div class="quickviewSliderThumb2 swiper-container position-relative">
                                ...
                                <div class="swiper-wrapper auto-item-width justify-content-center"
                                    ...
                                    <div class="swiper-slide position-relative aspect-1"
                                        onclick="focus_preview_image_by_color('preview-box-{{ str_replace('#','',$photo->color) }}')">
                                        <img src="{{asset(" storage/product/$photo->image_name")}}"
                                        onerror="this.src='{{theme_asset('assets/img/image-place-holder.png')}}'"
                                        class="dark-support rounded" alt="">
                                    </div>
                                    ...
                                    @if($photo->color == null)
                                    <div class="swiper-slide position-relative aspect-1"
                                        onclick="slider_thumb_img_preview('thumb_{{$key}}')">
                                        <img src="{{asset(" storage/product/$photo->image_name")}}"
                                        onerror="this.src='{{theme_asset('assets/img/image-place-holder.png')}}'"
                                        class="dark-support rounded" alt="">
                                    </div>
                                    ...
                                    @else
                                    @foreach (json_decode($product->images) as $key => $photo)
                                    <div class="swiper-slide position-relative aspect-1" id="preview-img{{$key}}"
                                        onclick="slider_thumb_img_preview('thumb_{{$key}}')">
                                        <img src="{{asset(" storage/product/".$photo)}}"
                                            onerror="this.src='{{theme_asset('assets/img/image-place-holder.png')}}'"
                                            class="dark-support rounded" alt="">
                                ...
                                <div class="swiper-button-next swiper-quickview-button-next" style="--size: 1.5rem">
                                </div>
                                <div class="swiper-button-prev swiper-quickview-button-prev" style="--size: 1.5rem">
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-lg-7">
                    ...
                    <div class="product-details-content position-relative">
                            ...
                            <div class="star-rating text-gold fs-12">
                                @for ($i = 1; $i <= 5; $i++) @if ($i <=(int)$overallRating[0]) <i
                                    ...
                                    @elseif ($overallRating[0] != 0 && $i <= (int)$overallRating[0] + 1.1 &&
                                        $overallRating[0]==((int)$overallRating[0]+.50)) <i class="bi bi-star-half"></i>
                                        @else
                        ...
                        @if(($product['product_type'] == 'physical') && ($product['current_stock']<=0)) <p
                            class="fw-semibold text-muted">{{translate('out_of_stock')}}</p>
                            @else
                            ...
                            <div class="product__price d-flex flex-wrap align-items-end gap-2 mb-4">
                                ...
                                @endif
                                <ins class="product__new-price fs-28 mb-n1">{{\App\CPU\Helpers::get_price_range($product)
                                    }}</ins>
                            </div>

                            <!-- Add to Cart Form -->
                            <form class="cart add_to_cart_form" id="add-to-cart-form" action="{{ route('cart.add') }}"
                                    ...
                                    <div class="d-flex gap-4 flex-wrap align-items-center mb-4">
                                        ...
                                        <div class="quantity quantity--style-two">
                                            ...
                                            <input type="text" class="quantity__qty product_quantity__qty"
                                                name="quantity" value="{{ $product->minimum_order_qty ?? 1 }}"
                                        ...
                                        <div class="flex-between-gap-3">
                                            <div class="">
                                                ...
                                                <h6 class="flex-middle-gap-2">
                                                    ...
                                                    <span
                                                        class="product_vat">{{\App\CPU\Helpers::currency_converter($product->tax)}}</span>
                                                </h6>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex gap-2 mt-4">
                                        ...
                                        $current_date >= $seller_vacation_start_date && $current_date <=
                                            ...
                                            $inhouse_vacation_start_date && $current_date <=
                                                ...
                                                class="buy_now_button btn btn-secondary fs-16" disabled>
                                                {{translate('buy_now')}}</span></button>
                                                <button type="button" class="update_cart_button btn btn-primary fs-16"
                                                    disabled>{{translate('add_to_Cart')}}</button>
                                                @else
                                                @php($guest_checkout=\App\CPU\Helpers::get_business_settings('guest_checkout'))
                                                <button type="button" class="buy_now_button btn btn-secondary fs-16"
                                                    onclick="buy_now('add-to-cart-form', {{($guest_checkout==1 ||Auth::guard('customer')->check()?'true':'false')}}, '{{route('shop-cart')}}')">{{translate('buy_now')}}</span></button>
