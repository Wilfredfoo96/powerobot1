...
$cart = \App\Model\Cart::where(['customer_id' => (auth('customer')->check() ? auth('customer')->id() :
session('guest_id'))])->with(['seller','all_product.category'])->get()->groupBy('cart_group_id');
@endphp
<div class="container">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-center mb-30">
                            <ul class="cart-step-list">
                                <li class="current cursor-pointer" onclick="location.href='{{route('shop-cart')}}'">
                                    <span><i class="bi bi-check2"></i></span> {{ translate('cart') }}</li>
                                <li class=" cursor-pointer" onclick="location.href='{{ route('checkout-details') }}'">
                        ...
                        <div class="bg-primary-light py-2 px-2 px-sm-3 mb-3 mb-sm-4">
                            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="ps-2 text-danger pulse-button" data-bs-toggle="tooltip"
                                        data-bs-placement="right" data-bs-custom-class="custom-tooltip"
                                        onclick="minimum_Order_Amount_message(this.getAttribute('data-bs-title'))"
                                ...
                                @if($physical_product && $shippingMethod=='sellerwise_shipping' && $shipping_type ==
                                'order_wise')
                                @php
                                ...
                                <div class="border bg-white rounded custom-ps-3">
                                    <div class="shiiping-method-btn d-flex gap-2 p-2 flex-wrap">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-left-auto"
                                                style="--bs-dropdown-min-width: 8rem">
                                                @foreach($shippings as $shipping)
                                                <li class="cursor-pointer"
                                                    onclick="set_shipping_id('{{$shipping['id']}}','{{$cartItem['cart_group_id']}}')">
                        ...
                        <div class="table-responsive d-none d-sm-block">
                                            ...
                                            <div class="media align-items-center gap-3">
                                                ...
                                                <div
                                                    ...
                                                    <h6 class="text-truncate text-capitalize" style="--width: 20ch">
                                                        <a
                                                            href="{{ $product->status == 1?route('product',$cartItem['slug']):'javascript:' }}">{{$cartItem['name']}}</a>
                                                    </h6>
                                                    @foreach(json_decode($cartItem['variations'],true) as $key1
                                                    =>$variation)
                                                    <div class="fs-12">{{$key1}} : {{$variation}}</div>
                                                    @endforeach
                                                    <div class="fs-12">{{ translate('Unit_Price') }} : {{
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="quantity quantity--style-two d-inline-flex">
                                                <span class="quantity__minus "
                                                    onclick="updateCartQuantityList('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '-1', '{{ $cartItem['quantity'] == $product->minimum_order_qty ? 'delete':'minus' }}')">
                                                    <i
                                                        class="{{ $cartItem['quantity'] == (isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1) ? 'bi bi-trash3-fill text-danger fs-10' : 'bi bi-dash' }}"></i>
                                                </span>
                                                <input type="text"
                                                    ...
                                                    onchange="updateCartQuantityList('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '0')"
                                                    data-min="{{ isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1 }}">
                                                <span class="quantity__plus"
                                                    onclick="updateCartQuantityList('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '1')">
                                            ...
                                            <div class="quantity quantity--style-two d-inline-flex">
                                                <span class="quantity__minus "
                                                    onclick="updateCartQuantityList('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '-{{$cartItem['quantity']}}', 'delete')">
                                                    <i class="bi bi-trash3-fill text-danger fs-10"></i>
                                                </span>
                                                <input type="hidden"
                                                    class="quantity__qty cartQuantity{{ $cartItem['id'] }}" value="1"
                                                    name="quantity[{{ $cartItem['id'] }}]"
                                                    id="cartQuantity{{$cartItem['id']}}" data-min="1">
                                            </div>
                                            @endif

                                        </td>
                                        <td class="text-end">{{
                                            \App\CPU\Helpers::currency_converter($cartItem['price']*$cartItem['quantity'])
                                            }}</td>
                                        <td class="text-end">{{
                                            \App\CPU\Helpers::currency_converter($cartItem['discount']*$cartItem['quantity'])
                                            }}</td>
                                        <td class="text-end">{{
                            ...
                            \App\CPU\OrderManager::free_delivery_order_amount($group[0]->cart_group_id))

                            @if ($free_delivery_status['status'] && (session()->missing('coupon_type') ||
                            session('coupon_type') !='free_delivery'))
                            <div class="free-delivery-area px-3 mb-3">
                                <div class="d-flex align-items-center gap-2">
                                    <img src="{{ asset('assets/front-end/img/icons/free-shipping.png') }}" alt=""
                                        width="40">
                                    @if ($free_delivery_status['amount_need'] <= 0) <span class="text-muted fs-16">{{
                                        translate('you_Get_Free_Delivery_Bonus') }}</span>
                                        @else
                                        <span class="need-for-free-delivery font-bold">{{
                                            \App\CPU\Helpers::currency_converter($free_delivery_status['amount_need'])
                                            }}</span>
                                        <span class="text-muted fs-16">{{ translate('add_more_for_free_delivery')
                                            }}</span>
                                        @endif
                                </div>
                        ...
                        <div class="d-flex flex-column d-sm-none">
                            ...
                            <div class="border-bottom d-flex align-items-start justify-content-between gap-2 py-2">
                                <div class="media gap-2">
                                    ...
                                    <div
                                        ...
                                        <div class="fs-12">{{ translate('shipping_cost') }} : {{
                                            \App\CPU\Helpers::currency_converter($cartItem['shipping_cost']) }}</div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="quantity quantity--style-two flex-column d-inline-flex">
                                    @if ($product->status == 1)
                                    <span class="quantity__minus "
                                        onclick="updateCartQuantityListMobile('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '-1', '{{ $cartItem['quantity'] == $product->minimum_order_qty ? 'delete':'minus' }}')">
                                        <i
                                            class="{{ $cartItem['quantity'] == (isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1) ? 'bi bi-trash3-fill text-danger fs-10' : 'bi bi-dash' }}"></i>
                                    </span>
                                    <input type="text" class="quantity__qty cartQuantity{{ $cartItem['id'] }}"
                                        value="{{$cartItem['quantity']}}" name="quantity[{{ $cartItem['id'] }}]"
                                        id="cartQuantityMobile{{$cartItem['id']}}"
                                        onchange="updateCartQuantityListMobile('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '0')"
                                        data-min="{{ isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1 }}">
                                    <span class="quantity__plus"
                                        onclick="updateCartQuantityListMobile('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '1')">
                                        <i class="bi bi-plus"></i>
                                    </span>
                                    @else
                                    <span class="quantity__minus "
                                        onclick="updateCartQuantityListMobile('{{ $product->minimum_order_qty }}', '{{$cartItem['id']}}', '-{{$cartItem['quantity']}}', 'delete')">
                                        <i class="bi bi-trash3-fill text-danger fs-10"></i>
                                    </span>
                                    <input type="hidden" class="quantity__qty cartQuantity{{ $cartItem['id'] }}"
                                        value="1" name="quantity[{{ $cartItem['id'] }}]"
                                        id="cartQuantityMobile{{$cartItem['id']}}" data-min="1">
                                    @endif
                                </div>
                            </div>
                            @endforeach
